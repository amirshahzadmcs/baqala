...
class Profile_controller extends CI_Controller
{
	...
	public function __construct()
	{
		...
		if ($this->teamleader->isLogged()) {
			...
			redirect("team_leader/login");
		}
	}

	public function change_password()
	{
		$data['result'] = $this->db->get_where('master_employee', ['id' => $this->teamleader->getId()])->row();
		// print_r($data);exit();
		$this->load->view('team_leader/auth/change_password', $data);
	}
	...
	public function submit_change_password()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->teamleader->checkPassword($hashpassword)) {
				$query = $this->db->update(
					'master_employee',
					[
						'password' => $hashpassword_new
					],
					['id' => $this->teamleader->getId()]
				);
				if ($query) {
		...
		redirect("team-leader/change-password");
	}

	public function profile()
	{
		$data['result'] = employeeDetailHelper($this->teamleader->getId());
		// dd($data);
		$this->load->view('team_leader/auth/profile', $data);
	}
}
