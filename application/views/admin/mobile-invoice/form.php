	...
	<div class="page-content-wrapper">
		<div class="row">
			<div class="col-12">
				<div class="card">
					<div class="card-body">
						<?php echo form_open("admin/mobile-invoice/submit", array("id" => "demo-form2", "enctype" => "multipart/form-data", "class" => "form-label-left", "data-parsley-validate" => "")); ?>
							<input type="hidden" id="id" name="id" value="<?php echo $id; ?>" />
								...
								<div class="row">
									...
									<div class="col-md-4 col-sm-12 mb-3 form-group">
										...
										<select name="sim_id" class="form-control select2" id="sim_id" onchange="checkDuplicateInvoice()" required>
											<option value="">-- Select Sim Card --</option>
											<?php if (!empty($sims)) {  
												foreach($sims as $key => $item) { ?>
													<?php
														if($item->status == '0'){
															$sim_status = '<span class="badge badge-pill badge-soft-primary font-size-13">New</span>';
														}
														if($item->status == '1'){
															$sim_status = '<span class="badge badge-pill badge-soft-success font-size-13">Active</span>';
														}
														if($item->status == '2'){
															$sim_status = '<span class="badge badge-pill badge-soft-warning font-size-13">Discontinued</span>';
														}
														if($item->status == '3'){
															$sim_status = '<span class="badge badge-pill badge-soft-danger font-size-13">Blocked</span>';
														}
														if($item->status == '4'){
															$sim_status = '<span class="badge badge-pill badge-soft-info font-size-13">Suspended</span>';
														}
														if($item->status == '5'){
															$sim_status = '<span class="badge badge-pill badge-soft-dark font-size-13">Free</span>';
														}
													?>
													<option value="<?php echo $sims[$key]->id; ?>" <?php echo ($sims[$key]->id == $sim_id) ? 'selected' : ''; ?>><?php echo $sims[$key]->mobile . ' ('.$item->sim_no.')'; ?> - <?php echo $sim_status ?></option>
												<?php } } else { ?>
									...
									<div class="col-md-4 col-sm-12 mb-3 form-group">
										...
										<div class="col-md-12 mt-2">
											...
											<div class="float-start text-center image-container mt-2 me-2" id="img_<?php echo $cv_doc['id'];?>">
												...
												<div class="edit"><a href="<?php echo base_url($cv_doc['file_name']);?>" class="btn btn-primary btn-sm" target="_blank" title="View"><i class="fa fa-eye"></i></a> <a type="button" class="btn btn-danger btn-sm remove-cv-image" data-id="<?php echo $cv_doc['id'];?>" title="Delete"><i class="fa fa-trash"></i></a></div>
											</div>
											<?php }} ?>
										</div>
									</div>
