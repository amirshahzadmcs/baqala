		...
		<div class="row align-items-center">
			<div class="col-sm-6">
				<div class="page-title">
					...
					<ol class="breadcrumb m-0">
						...
						<li class="breadcrumb-item"><a href="<?php echo base_url('admin/logistic-management/new-jahez/index'); ?>">Jahez Orders</a></li>
			...
			<div class="col-sm-6">
				<div class="float-end d-sm-block">
					<?php if (check_action_permission(get_user_role(), 'new_daily_performance', 'delete')): ?>
						<div class="btn-group">
					...
					<?php endif;
					if (check_action_permission(get_user_role(), 'new_daily_performance', 'import_file')): ?>
					...
					<div class="btn-group ms-2 float-end">
						...
						<div class="dropdown-menu dropdown-menu-end">
							<h6 class="dropdown-header">EXPORT AS</h6>
							<a type="button" class="dropdown-item" data-export_modal="daily_performance" title="Daily Performance Report">Daily Performance Report</a>
							<div class="dropdown-divider"></div>
							<a type="button" class="dropdown-item" data-export_modal="daywise" title="Day Wise Report">Day Wise Report</a>
							<div class="dropdown-divider"></div>
							<a type="button" class="dropdown-item" data-export_modal="weekly" title="Weekly Report">Weekly Performance Report</a>
							<div class="dropdown-divider"></div>
							<a type="button" class="dropdown-item" data-export_modal="monthly" title="Monthly Performance Report">Monthly Performance Report</a>
							<div class="dropdown-divider"></div>
							<a type="button" class="dropdown-item" data-export_modal="revenue" title="Monthly Revenue Report">Monthly Revenue Report</a>
							<div class="dropdown-divider"></div>
							<a type="button" class="dropdown-item" data-export_modal="cash_summary" title="Cash Collection Report">Cash Collection Report</a>
						</div>
					</div>
				</div>
				...
				<form id="delivery_bulk_form" method="post" enctype="multipart/form-data" autocomplete="off">
					<div class="row px-3">
						...
						<div class="col-md-12">
							...
							<p style="text-align: center;"><a type="button" href="<?php echo base_url('admin_assets/samples/Jahez_New_Sample_File.xlsx'); ?>" class="btn bt btn-link text-success" target="_blank" download>Download sample file</a></p>
						</div>
	...
	<div class="modal-dialog modal-lg modal-dialog-centered" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<form id="exportForm" action="<?php echo base_url('admin/logistic-management/noon/print-order-summary'); ?>" method="POST">
...
<div class="modal fade staticBackdrop fixed-left filtersModal" id="filtersModal" data-bs-backdrop="static" data-bs-keyboard="false" aria-labelledby="#filtersModalLabel" style="display: none;" aria-hidden="true">
	<div class="modal-dialog modal-dialog-aside">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="<?php echo base_url('admin/logistic-management/new-jahez/list'); ?>" method="get" id="filter_form">
					<div class="row">
...
<script>
	...
	$(document).ready(function() {
		...
		function initializeDataTable() {
			...
			table = $('#summaryTable').DataTable({
				...
				"processing": true,
				"ajax": {
					url: "<?php echo base_url(); ?>admin/logistic-management/new-jahez/ajax-list",
					type: "POST",
					data: function(d) {
						...
						d.vehicle_type = $('select[name="vehicle_type"] option:selected').val();
  						d.team = $('select[name="team"] option:selected').val();
						d.vehicle_no = $('input[name="vehicle_no"]').val();
		...
		$(document).ready(function() {
			...
			function checkFilters() {
				...
				if (search || dateFrom || dateTo || vehicleType || vehicleNo || team) {
					$('#resetFilters').show();
				} else {
					$('#resetFilters').hide();
				}
			...
			$('#searchInput, #dateFrom, #dateTo, #vehicle_type, #vehicle_no, #team').on('input change', function() {
				checkFilters();
			});
			...
			$('#resetFilters').click(function() {
				...
				// Reset the DataTable to the first page
				currentPage = 0;
		...
		$("body").on("submit", "#delivery_bulk_form", function(e) {
			...
			$.ajax({
				type: 'POST',
				url: "<?php echo base_url('admin/logistic-management/new-jahez/import') ?>",
				data: data,
				//dataType: 'json',
				contentType: false,
				cache: false,
	...
	function getColumnForm() {
		$.ajax({
			...
			method: 'GET',
			data: {
				request_type: 'jahez_order_summary'
			},
			success: function(response) {
				try {
	...
	function deleteAction() {
		const inputCount = $('[name="checklist[]"]:checked').length;

		if (inputCount > 0) {
			Swal.fire({
				title: 'Are you sure?',
				text: `You are about to delete ${inputCount} item(s). This action cannot be undone!`,
				icon: 'warning',
				...
				if (result.isConfirmed) {
					changeActionAndSubmit('admin/logistic-management/new-jahez/delete');
				} else {
					Swal.fire('Cancelled', 'Your data is safe 🙂', 'info');
				}
			});
		} else {
			Swal.fire('No Selection', 'Please select at least one checkbox.', 'warning');
		}
	}
			...
			if (result.isConfirmed) {
				...
				Swal.fire({
					...
					if (confirmResult.isConfirmed) {
						...
						$.ajax({
							url: '<?= base_url("admin/logistic-management/new-jahez/delete-datewise") ?>',
							type: 'POST',
							data: { date: date },
							success: function(response) {
								if (response.status) {
	...
	$(document).ready(function() {
		...
		$('#exportForm').on('submit', function(e) {
			...
			let exportUrl = (fileFormat === 'file_format_xlsx') ?
				'admin/logistic-management/new-jahez/export-daily-summary' :
				'admin/logistic-management/new-jahez/print-order-summary';
	...
	$(document).ready(function () {
		$('[data-export_modal]').on('click', function () {
			...
			$.ajax({
				url: "<?= base_url('admin/logistic-management/NewJahez/print_modal_form/') ?>" + modalName,
				type: "GET",
				success: function (response) {
					$('#filterModal .modal-content').html(response);
				},
				error: function () {
					$('#filterModal .modal-content').html('<div class="p-4 text-danger text-center">Failed to load content.</div>');
				}
			});
		});
	});
