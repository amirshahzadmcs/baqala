				...
				<div class="card">
					<div class="card-body">
						...
						<div class="row size-inner-section px-2 py-4 mx-2">
							<div class="w-50"><h4 class="header-title float-start">Alloted/Unalloted Assets List</h4></div>
							<?php if (!empty($assets_logs)) : ?>
							<div class="teamMembers table-responsive">
								
							</div>
							<?php else : ?>
							<div class="text-center m-auto">
								<p><i class="dripicons-box text-secondary fa-3x"></i></p>
								<p>No Assets Log Found</p>
							</div>
							<?php endif; ?>
						</div>
						
						<div class="row size-inner-section px-2 py-4 mx-2">
							<div class="w-50"><h4 class="header-title float-start">Alloted/Unalloted Sim List</h4></div>
							<?php if (!empty($sim_logs)) : ?>
							<div class="sim-logs table-responsive">
								<table id="simLogTable" class="table border">
									<thead>
										<tr>
											<th>S.No.</th>
											<th>Sim No.</th>
									...
									<tbody>
										<?php $sno1=1;foreach($sim_logs as $sim_log){ ?>
										<tr>
											<td><?php echo $sno1++;?></td>
											<td><?= $sim_log['sim_no'];?></td>
											<td><?= $sim_log['mobile'];?></td>
											<td><?= ucfirst($sim_log['sim_type']);?></td>
											<td><?= $sim_log['network_name'];?></td>
											<td><?= date('d-m-Y', strtotime($sim_log['allot_date']));?></td>
											<td><?= ($sim_log['unallot_date'] ? date('d-m-Y', strtotime($sim_log['unallot_date'])) : '<span class="text-success">Still Allotted</span>'); ?></td>
										</tr>
										<?php } ?>
									</tbody>
								</table>
						...
						<div class="twitter-bs-wizard">
							<ul class="pager wizard twitter-bs-wizard-pager-link">
								<li class="previous"><a href="<?php echo ($emp_detail->id > 0) ? base_url('admin/hr/employees/view/step-9/'.$emp_detail->id) : base_url('admin/hr/employees'); ?>" class="btn btn-custom-secondary"><i class="mdi mdi-arrow-left me-1"></i> Prevoius </a></li>
							</ul>
						</div>
					</div>
				</div>
			...
			buttons: [
				{
					extend: 'excel',
					title: 'SIM Report',
					messageTop: 'Alloted and Unalloted SIM List of: ' + employeeInfo
				},
				{
					extend: 'pdf',
					title: 'SIM Report',
				...
				},
				{
					...
					title: 'SIM Report',
					messageTop: function () {
						return 'Alloted and Unalloted SIM List of: ' + employeeInfo;
					}
				},
				'colvis'
			],
