...
class Hunger_sales_invoice extends CI_Controller
{
	...
	public function __construct()
	{
		...
		// Load Model
		$this->load->model('admin/logistic-management/aggregators/hunger/Hunger_sales_invoice_model', 'hunger_invoice_model');
	...
	{
		if ($this->action && !check_action_permission(get_user_role(), 'sales_invoice', $this->action)) {
		...
		//$data['hunger_summary'] = $this->hunger_invoice_model->getSummary();
		...
		$this->load->view("admin/logistic-management/aggregators/hunger_invoice/index", $data);
	}
	...
	public function ajax_list()
	{
		...
		$fetch_data = $this->hunger_invoice_model->salesList($search, $perPage, $start);
	...
	public function import_file()
	{
		...
		try {
			...
			$existing_records = $this->hunger_invoice_model->checkDuplicateInBulk($summary_date);
				...
				foreach ($sheet_data as $val) {
					if (!empty($val[0])) { // Replace with necessary column checks
						$batch_data[] = [
							...
							"basic_payment" => $val[4],
							"acceptance_rate_penalties" => $val[5],
							"contact_rate_penalties" => $val[6],
							"stacking_deduction" => $val[7],
							"declined_penalties_day_logic" => $val[8],
							"google_capped_after" => $val[9],
							"amount_exlc_vat_before_bonus" => $val[10],
							"3pl_bonus" => $val[11],
							"amount_exlc_vat_with_bonus" => $val[12],
							"vat" => $val[13],
							"amount_incl_vat" => $val[14],
							"rider_scoring" => $val[15],
							"rider_basic" => $val[16],
							"monthly_balance" => $val[17],
							"total_deduction" => $val[18],
							"net_ftr" => $val[19],
							'added_by' => $userId,
	...
	{
		$result = $this->db->insert_batch('maha_hunger_sales_invoice', $batch_data);
	...
	public function print_sales_data()
	{
		...
		$data['report_data'] = $this->hunger_invoice_model->printSalesReport($column_type, $file_format, $search);
		...
		$pdf->SetTitle('Hunger Monthly Sales Invoice -' . $data['month_of']);
		$pdf->SetSubject('Hunger Monthly Sales Invoice -' . $data['month_of']);
		$pdf->SetKeywords('Maha Al Fala, PDF, Hunger Monthly Sales Invoice');

		// print_r($data);exit();
		// remove default header/footer
		$pdf->setPrintHeader(true);
		$pdf->SetPrintFooter(true);
		$htmlHeader = $this->load->view('admin/logistic-management/aggregators/hunger_invoice/print/header', $data, true);
		$htmlHeader2 = $this->load->view('admin/logistic-management/aggregators/hunger_invoice/print/header', $data, true);
		$pdf->setHtmlHeader($htmlHeader);
		$pdf->setHtmlHeader2($htmlHeader2);

		$lastFooter = $this->load->view('admin/logistic-management/aggregators/hunger_invoice/print/footer', $data, true);
		...
		// Arabic and English content
		$htmlcontent = $this->load->view('admin/logistic-management/aggregators/hunger_invoice/print/report', $data, true);
		$pdf->WriteHTML($htmlcontent, true, 0, true, 0);
		//Close and output PDF document
		$pdf->Output('BS-Hunger-Station-Sales-Invoice.pdf', 'I');
	...
	public function export_sales_data()
	{
		...
		$search = $this->input->post('searched_value');
		$file_format = $this->input->post('file_format');
		$reportData = $this->hunger_invoice_model->printSalesReport($column_type, $file_format, $search);
		...
		$filename = "Hunger-Sales-Invoice.xlsx";
		$spreadsheet = new Spreadsheet();
		$sheet = $spreadsheet->getActiveSheet();

		// Report Title
		$sheet->mergeCells('A1:H1');
		$sheet->setCellValue('A1', 'Hunger Monthly Sales Invoice');
