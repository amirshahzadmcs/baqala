...
class Deliveryvehicle_model extends CI_Model
{
    ...
    public function add_basic($hashpassword)
    {
        ...
        $query = $this->db->query("INSERT INTO delivery_vehicles SET 
			...
			rider_type = '" . $this->db->escape_str($this->input->post('rider_type')) . "', 
    ...
    {
        $query = $this->db->query("SELECT db.*, mb.bank_name as bankname, mc.color_name, mp.profession_name, mvk.make_name, sa.area_name, dp.company_name as logistic_partner FROM delivery_vehicles db LEFT JOIN master_bank mb ON (mb.id = db.bank_name) LEFT JOIN master_color mc ON (mc.id = db.van_color) LEFT JOIN master_profession mp ON (mp.id = db.profession) LEFT JOIN mater_van_make mvk ON (mvk.id = db.van_make) LEFT JOIN service_area sa ON (sa.id = db.service_area) LEFT JOIN delivery_partner dp ON (db.partner_id = dp.id) WHERE db.id = '" . (int) $id . "'");
        return $query;
    }
    ...
    {
        $a = "SELECT db.*, (SELECT IF(lp.id > 0, lp.company_name, 'N/A') FROM delivery_partner lp WHERE db.partner_id = lp.id) as partner_name FROM delivery_vehicles db WHERE 1 = 1";
        return $a;
    }
    
    public function get_list($keyword,$status,$application_status,$rider_type,$partner_id,$iqama_no,$plate_no,$from,$to)
    {
        $a = $this->make_query();
		if($keyword){
			$a .= " AND (db.name LIKE '%".$keyword."%' OR db.email LIKE '%".$keyword."%' OR db.driver_id LIKE '%".$keyword."%')";
		}
		if($status){
			if($status == 'inactive'){
				$a .= " AND db.status = '0'";
		...
		if($application_status){
			$a .= " AND db.application_status = '" . $application_status . "'";
		}
		if($rider_type){
			$a .= " AND db.rider_type = '" . $rider_type . "'";
		}
		if($partner_id){
			$a .= " AND db.partner_id = '" . $partner_id . "'";
		}
		if($iqama_no){
			$a .= " AND db.iqama_no = '" . $iqama_no . "'";
		}
		if($plate_no){
			$a .= " AND db.van_no = '" . $plate_no . "'";
		}
		if($this->input->get('from') AND $this->input->get('to')){
			...
			if($v_from AND $v_to){
				$a .= " AND (db.created_at BETWEEN '". $v_from ."' AND '". $d_to ."')";
			}
		}
        // if (isset($_POST["search"]["value"])) {
        //     $a .= " AND db.name LIKE '%" . $_POST["search"]["value"] . "%' OR db.iqama_no LIKE '%" . $_POST["search"]["value"] . "%' OR db.van_no LIKE '%" . $_POST["search"]["value"] . "%' OR db.driver_id LIKE '%" . $_POST["search"]["value"] . "%' OR db.email LIKE '%" . $_POST["search"]["value"] . "%'";
        // }
    ...
    public function get_filtered_data($keyword,$status,$application_status,$rider_type,$partner_id,$iqama_no,$plate_no,$from,$to)
    {
        $a = $this->make_query();
		if($keyword){
			$a .= " AND (db.name LIKE '%".$keyword."%' OR db.email LIKE '%".$keyword."%' OR db.driver_id LIKE '%".$keyword."%')";
	...
	{
		$query = $this->db->query("SELECT dv.*, dp.company_name, dp.company_arabic_name as ar_name, dp.name as contact_p, sr.area_name as area_name, vm.make_name FROM delivery_vehicles dv, delivery_partner dp, service_area sr, mater_van_make vm WHERE dv.id = '" . (int)$id . "' AND dv.partner_id = dp.id AND sr.id = dv.service_area AND vm.id = dv.van_make");
		return $query->row();
	}
	
	public function getRiders()
	{
		$query = $this->db->query("SELECT name,id FROM delivery_vehicles WHERE status = '1' AND application_status = 'verified'");
		return $query->result();
	}

	public function getUnallotedRiders()
	{
		$query = $this->db->query("SELECT dv.name,dv.id FROM delivery_vehicles dv WHERE dv.status = '1' AND dv.id NOT IN (SELECT sim.user_no from sim_allot as sim WHERE sim.status=1) AND application_status = 'verified'");
		return $query->result();
	}
