		...
		<div class="row align-items-center">
			<div class="col-sm-6">
				<div class="page-title">
					<h4>Block Reason Master</h4>
					<ol class="breadcrumb m-0">
						<li class="breadcrumb-item"><a href="<?php echo base_url('admin'); ?>">Dashboard</a></li>
						<li class="breadcrumb-item active">Block Reason List</li>
					</ol>
				</div>
			</div>
			<div class="col-sm-6">
				<div class="float-end d-sm-block">
					<?php if (check_action_permission(get_user_role(), 'block_reasons', 'delete')): ?>
					...
					<?php endif;
					if (check_action_permission(get_user_role(), 'block_reasons', 'add')): ?>
						&nbsp;
						<a class="btn btn-custom-success btn-sm pull-right me-1" title="Own Store" href="<?php echo base_url('admin/block-reasons/add') ?>"><i class="fa fa-plus"></i> Add Block Reason</a>
					<?php endif; ?>
				</div>
...
<script>
	$(document).ready(function() {
		$('#store-table').dataTable({
			...
			"fixedHeader": true,
			"ajax": {
				url: "<?php echo base_url(); ?>admin/new/BlockReason/get_list",
				type: "POST"
			},
			"columnDefs": [{
				"targets": [0, 1, 2, 3, 4, 5],
	...
	function deleteAction() {
		...
		if (inputCount > 0) {
			if (confirm("Do you want to delete selected resons?") == true) {
				changeActionAndSubmit('admin/block-reasons/delete');
			} else {
