        ...
        <div class="row align-items-center">
            ...
            <div class="col-sm-6">
                <div class="float-end d-sm-block">
                    <button type="button" class="btn btn-custom-success btn-sm pull-right" title="Add" id="addLeaveBtn"><i class="fa fa-plus"></i> Add Approval Cycle</button>
                </div>
								...
								<tr>
									...
									<td class="p-0">
										<table class="table custom-hover-table" cellpadding="5" cellspacing="0">
											...
											<tbody>
												<?php 
												foreach ($leave_approval as $result) {
												...
												?>
													<tr>
														...
														<td>
															<?php $approvalDetail = json_decode($result->approval_detail);?>
															<span class="badge rounded-pill badge-soft-secondary text-dark px-3 py-2">
																<?= $approvalDetail->leave_types == 0 ? 'All Leave Types' : $result->leave_type_name; ?>
															</span>
														</td>
    ...
    <div class="modal-dialog modal-dialog-aside">
		<div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title mt-0" id="leaveModalLabel">New Leave Approval Cycles</h5>
...
$('#addLeaveBtn').on('click', function() {
    ...
    $.ajax({
        url: '<?php echo base_url('admin/hr-module/request/add-leave-form'); ?>',
        method: 'GET',
        success: function(responseHtml) {
			...
			//initializeSortable();
			$('#leaveModalLabel').html('Add Leave Approval Cycles');
			var footerHtml = `<div class="row">
					<div class="col-md-12">
						<button form="addLeaveForm" type="submit" class="btn btn-success btn-md">Save</button>
...
$('.edit-leave-btn').on('click', function() {
    ...
    // Make the AJAX request
    $.ajax({
        url: '<?php echo base_url('admin/hr-module/request/edit-leave-form'); ?>',
        method: 'GET',
        ...
        success: function(responseHtml) {
			...
			$('.leave-modal-body').html(responseHtml);
			$('.select2').select2({dropdownCssClass: 'custom-dropdown'});
			$('#leaveModalLabel').html('Edit Leave Approval Cycles');
			var footerHtml = `<div class="row">
					<div class="col-md-12">
						<button form="editLeaveForm" type="submit" class="btn btn-success btn-md">Save</button>
						<button type="button" data-bs-dismiss="modal" aria-label="Close" class="btn btn-outline-danger btn-md ms-2">Cancel </button>
					</div>
				</div>`;
			$('.leaveModal .modal-footer').html(footerHtml);
...
$(document).on('click', '.delete-btn2', function() {
	...
	Swal.fire({
		...
		if (result.isConfirmed) {
			$.ajax({
				url: '<?php echo base_url('admin/hr-module/request/delete-leave'); ?>',
				method: 'POST',
