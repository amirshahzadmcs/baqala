		...
		<div class="row align-items-center">
			<div class="col-sm-6">
				<div class="page-title">
					<h4>Hunger Monthly Performance</h4>
					<ol class="breadcrumb m-0">
						<li class="breadcrumb-item"><a href="<?php echo base_url('admin'); ?>">Dashboard</a></li>
						<li class="breadcrumb-item"><a href="<?php echo base_url('admin/logistic-management/invoices/hunger/list'); ?>">Hunger Monthly Performance</a></li>
			...
			<div class="col-sm-6">
				<div class="float-end d-sm-block">
					<?php if(check_action_permission(get_user_role(), 'monthly_performance', 'import_file')):?>
						...
						<table id="employeeTable" class="table table-bordered jambo_table" style="width:100%">
							<thead>
								<tr>
									...
									<th class="text-center">Notified Deliveries</th>
									<th class="text-center">Completed Deliveries</th>
							...
							<tbody>
								...
								?>
										<tr>
											...
											<td align="center"><?= $summary['Total_Notified_Deliveries']; ?></td>
											<td align="center"><?= $summary['Total_Completed_Deliveries']; ?></td>
											<td align="center">
											...
											<td align="center">
												<div class="btn-group ms-2">
													...
													<div class="dropdown-menu dropdown-menu-end">
														<?php if (check_action_permission(get_user_role(), 'monthly_performance', 'detail')): ?>
															<a class="dropdown-item" href="<?= base_url('admin/logistic-management/invoices/hunger/detail/' . $summary['id']); ?>">
																<i class="mdi mdi-stretch-to-page-outline me-2"></i> Detail
															</a>
														<?php endif;
														if (check_action_permission(get_user_role(), 'monthly_performance', 'print_monthly_performance') || check_action_permission(get_user_role(), 'monthly_performance', 'export_monthly_performance')): ?>
															<div class="dropdown-divider"></div>
															<a type="button" class="dropdown-item export-btn" title="Monthly Report" data-bs-toggle="modal" data-bs-target="#exportModal" data-requestid="<?= $summary['id']; ?>">
																<i class="mdi mdi-download me-2"></i> Download
															</a>
														<?php endif; ?>
														<?php if (check_action_permission(get_user_role(), 'monthly_performance', 'delete')): ?>
															<div class="dropdown-divider"></div>
															<a href="javascript:void(0);" class="dropdown-item delete-summary-btn" title="Delete Report" data-requestid="<?= $summary['id']; ?>">
	...
	<div class="modal-dialog modal-dialog-centered" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title px-3" id="bulkImportModalLabel">Rider Monthly Performance</h5>
			...
			<div class="modal-body">
				...
				<form id="delivery_bulk_form" method="post" enctype="multipart/form-data" autocomplete="off">
					<div class="row px-3">
						...
						<div class="col-md-12 col-sm-12 mb-3 form-group">
							<label for="attachment">Monthly Performance File <span class="text-secondary">(Supported file format is: .xls .xlsx)</span></label>
						...
						<div class="col-md-12">
							...
							<p style="text-align: center;"><a type="button" href="<?php echo base_url('admin_assets/samples/Hunger-Monthly-Performance.xlsx'); ?>" class="btn bt btn-link text-success" target="_blank" download>Download sample file</a></p>
						</div>
	...
	<div class="modal-dialog modal-lg modal-dialog-centered" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<form id="exportForm" action="<?php echo base_url('admin/logistic-management/invoices/hunger/print-monthly-performance'); ?>" method="POST">
					...
					<div class="row">
						<?php if (check_action_permission(get_user_role(), 'monthly_performance', 'print_monthly_performance')): ?>
							<div class="col-md-12">
								<div class="form-check mb-3">
						...
						<?php endif;
						if (check_action_permission(get_user_role(), 'monthly_performance', 'export_monthly_performance')): ?>
							<div class="col-md-12">
								<div class="form-check mb-3">
...
<script>
	...
	$(document).ready(function() {
		$("body").on("submit", "#delivery_bulk_form", function(e) {
			...
			$.ajax({
				type: 'POST',
				url: "<?php echo base_url('admin/logistic-management/invoices/hunger/import-file') ?>",
				data: data,
				//dataType: 'json',
				contentType: false,
				cache: false,
	...
	$(document).ready(function() {
		...
		$('#exportForm').on('submit', function(e) {
			...
			let exportUrl = (fileFormat === 'file_format_xlsx') ?
				'admin/logistic-management/invoices/hunger/export-monthly-performance' :
				'admin/logistic-management/invoices/hunger/print-monthly-performance';
	...
	$(document).on('click', '.delete-summary-btn', function () {
		var id = $(this).data('requestid');

		Swal.fire({
			title: 'Are you sure?',
			text: "This will delete the report and all related records!",
			icon: 'warning',
			...
			if (result.isConfirmed) {
				$.ajax({
					url: '<?= base_url("admin/logistic-management/invoices/hunger/delete") ?>',
					type: 'POST',
					data: { id: id },
					dataType: 'json',
					success: function (response) {
