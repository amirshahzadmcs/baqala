			...
			<div class="col-sm-6">
				<div class="float-end d-sm-block">
					...
					if (check_action_permission(get_user_role(), 'accident_list', 'add')): ?>
						<a class="btn btn-custom-success btn-sm pull-right me-2" title="New Employee" href="<?php echo base_url('admin/hr/accidents/add') ?>"><i class="fa fa-plus"></i> New Accident</a>
					<?php endif; ?>
				</div>
		...
		<div class="row">
			<div class="col-12">
				<div class="card">
					...
					<div class="card-body">
						<form action="<?php echo base_url('admin/hr/accidents'); ?>" method="get" id="filter_form">
							<div class="row">
							...
							<div class="row mt-2">
								<div class="col-lg-12 col-md-12 col-sm-12">
									...
									<a href="<?php echo base_url('admin/hr/accidents'); ?>" class="btn btn-danger btn-md float-end">Reset Filter</a>
								</div>
							</div>
						</form>
					</div>
...
<script>
	$(document).ready(function() {
		$('#accidentTable').dataTable({
			...
			"fixedHeader": true,
			"ajax": {
				url: "<?php echo base_url(); ?>admin/hr/accidents/ajax-list?keyword=<?php echo $this->input->get('keyword') ?>&vehicle_no=<?php echo $this->input->get('vehicle_no') ?>&insurance_provider=<?php echo $this->input->get('insurance_provider') ?>",
				type: "POST"
			},
			"columnDefs": [{
	...
	function deleteAction() {
		...
		if (inputCount > 0) {
			if (confirm("Do you want to delete selected employee?") == true) {
				changeActionAndSubmit('admin/hr/accidents/delete');
			} else {
