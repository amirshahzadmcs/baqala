...
class SimCard extends CI_Controller {
	...
	public function __construct() {
		...
		if($this->admin->isLogged()){
			...
			$this->load->model('admin/hr/recruitment/Sim_card_model');
	...
	public function index()
	{
		...
		$this->load->view('admin/hr/recruitment/sim_card/index',$data);
	}

	
	public function add(){
		if($this->input->get('id')){
			$query = $this->Sim_card_model->get_detail($this->input->get('id'));
			$data['id'] = $query->id;
		...
		$this->load->view('admin/hr/recruitment/sim_card/form',$data);
	}
	...
	public function save(){
		...
		if($this->form_validation->run()==FALSE){
    		...
    		if($this->input->post('id')){
    			$query = $this->Sim_card_model->edit();
    		}
    		else{
    			$query = $this->Sim_card_model->add();
    		}
    		if($query){
		...
		redirect('admin/hr/recruitment/sim-card');
	}
	...
	public function detail(){
		if($this->input->get('id')){
			...
			// print_r($data['networks']);die();
			$data['sims'] = $this->Sim_model->get_list();
			$this->load->view('admin/hr/recruitment/sim_card/detail',$data);
		}else{
	...
	public function get_list(){
		$fetch_data = $this->Sim_card_model->get_list();
// print_r($fetch_data);die();
		...
		foreach($fetch_data as $store){
			...
			$sub_array[] = '<a class="btn btn-outline-secondary btn-custom-light btn-sm edit" title="Edit" href="'.base_url().'admin/hr/recruitment/sim-card/add?id='.$store->id.'"><i class="mdi mdi-pencil font-size-18"></i></a> <a class="btn btn-outline-secondary btn-custom-light btn-sm edit" title="Detail" href="'.base_url().'admin/hr/recruitment/sim-card/detail?id='.$store->id.'"><i class="mdi mdi-stretch-to-page-outline font-size-18"></i></a>';
			
			$data[] = $sub_array;
		}						
		$output = array(  
			"draw"                =>     intval($_POST["draw"]),  
			"recordsTotal"        =>      $this->Sim_card_model->get_all_data(),  
			"recordsFiltered"     =>     $this->Sim_card_model->get_filtered_data(),  
			"data"                =>     $data  
		);  
	...
	public function delete(){
		...
		$query = $this->Sim_card_model->delete($ids);
		if($query){
	...
	public function get_emp_detail()
	{
		...
		$data = $this->Sim_card_model->get_emp_detail($id);
