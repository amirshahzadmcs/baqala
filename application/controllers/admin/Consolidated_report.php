...
class Consolidated_report extends CI_Controller {

	public function __construct() {
		parent::__construct();									
		if($this->admin->isLogged()){		
			$this->load->model('admin/Consolidated_model');
	...
	public function index(){
		...
		$this->load->view('admin/consolidated/report',$data);
	}
	
	public function report(){
		$data['purchase_report'] = $this->Consolidated_model->get_purchase_report();
		$data['purchase_return'] = $this->Consolidated_model->get_po_return_report();
		$data['cash_sales'] = $this->Consolidated_model->get_cash_sales();
		$data['credit_sales'] = $this->Consolidated_model->get_credit_sales();
		$data['return_sales'] = $this->Consolidated_model->get_return_sales();
		$data['expenses_report'] = $this->Consolidated_model->get_expenses_list();
		if ($this->admin->getInfo()){
			$info = explode('--', $this->admin->getInfo());
			$data['info'] = $info[1];
			$data['info_type'] = $info[0];
	...
	public function report_new(){
		...
		//print_r($data['cash_sales']);exit();
		$this->load->view('admin/consolidated/report2',$data);
	}
