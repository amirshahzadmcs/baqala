			...
			<div class="col-sm-6">
				<div class="page-title">
					...
					<ol class="breadcrumb m-0">
						...
						<li class="breadcrumb-item active">Document Rejection List</li>
					</ol>
				</div>
			</div>
			<div class="col-sm-6">
				<div class="float-end d-sm-block">
					<?php if (check_action_permission(get_user_role(), 'document_rejections', 'delete')): ?>
					...
					<?php endif;
					if (check_action_permission(get_user_role(), 'document_rejections', 'add')): ?>
						&nbsp;
						<a class="btn btn-custom-success btn-sm pull-right me-1" title="Own Store" href="<?php echo base_url('admin/doc-rejection-reasons/add') ?>"><i class="fa fa-plus"></i> Add Document Rejection</a>
					<?php endif; ?>
				</div>
...
<script>
	$(document).ready(function() {
		$('#store-table').dataTable({
			...
			"fixedHeader": true,
			"ajax": {
				url: "<?php echo base_url(); ?>admin/new/DocRejectionReason/get_list",
				type: "POST"
			},
			"columnDefs": [{
				"targets": [0, 1, 2, 3, 4, 5],
	...
	function deleteAction() {
		...
		if (inputCount > 0) {
			if (confirm("Do you want to delete selected resons?") == true) {
				changeActionAndSubmit('admin/doc-rejection-reasons/delete');
			} else {
