...
<?php 
 $cat_id = $current_detail['id'];
 $breadcrumb = $this->admin->getBreadcrumbTree($cat_id);
 $parent_ids = array_column($breadcrumb['categories'], 'id');
 //print_r($parent_ids);
?>
<!-- start page title -->
			...
			<div class="col-sm-6">
				<div class="page-title">
					...
					<ol class="breadcrumb m-0">
						...
						<li class="breadcrumb-item"><a href="<?php echo base_url('admin/chart-of-accounts/');?>">Chart of Accounts</a></li>
			...
			<div class="col-12">
				<div class="card mb-2">
					<div class="card-body p-0">
						<div class="header__pages headershow d-block" style="z-index: 1; top: 45px;">
                            <div class="container-fluid">
                                <div class="row align-items-center">
                                    <div class="col-sm-8">
                                        <div class="row align-items-center">
                                            <div class="col-sm-12 head-info">
                                                <div class="pages-head-title">
                                                    <h2 class="d-flex align-items-center"><?php echo $parent_detail['name'];?> <span class="fs-14">#<?php echo $parent_detail['id'];?></span>
                                                    <div class="dropdown">
														<button type="button" class="btn dropdown-toggle"  data-bs-toggle="dropdown">
															<i class="fas fa-cog fs-12"></i>
														</button>
														<ul class="dropdown-menu">
															<li><a target="_blank" class="dropdown-item" href="<?php echo base_url('admin/accounting/costcenter/folder-view'); ?>"><i class="fas fa-eye text-success"></i> View</a></li>
															<li><a data-bs-toggle="modal" data-bs-target="#editheaderinnermodel" class="dropdown-item" href="javascript:;"><i class="mdi text-info mdi-pencil-box-outline"></i> Edit</a></li>
														</ul>
                                                    </div>
                                                    </h2>
                                                    <div class="route-links"><span><?php echo $parent_detail['name'];?><span class="tip-circle lazy-tip tooltipstered non observed" data-title="assets-cat"><i class="fas fa-question-circle" style="margin-left:5px;"></i></span></span></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-sm-4 text_rightcol">
                                        <div class="credit-wrap text-right d-inline-block align-middle">
                                            <div class="credit-container px-2 credit-overdue" style="border-color: rgb(14, 132, 84);">
                                                <p class="cost">SAR&nbsp;0</p>
                                                <p class="type"><?php echo ucfirst($parent_detail['is_parent']);?></p>
                                            </div>
                                        </div>
                                        <div class="action-buttons d-inline-block">
                                            <div class="btn-group ml-3" role="group" aria-label="padination">
                                                <a target="_blank" href="<?php echo base_url('admin/accounting/costcenter/cost-transactions') ?>" class="btn btn-secondary"><i class="fas fa-chart-pie"></i></a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
								...
								<div class="chart-of-accounts-col col-md-4 border chart-of-accounts-col-3 px-0">
									...
									<div class="chart-of-accounts-col-3-body-container">
										<?php foreach($chart_list as $clist){ ?>
											...
											<li>
												<span class="caret <?php (in_array($clist['id'], $parent_ids)) ? ' caret-down' : '';?>" onclick="quickSidebar(this)" data-id="<?php echo $clist['id'];?>"><a href="<?php echo base_url('admin/cost-center/cats/'.$clist['id']); ?>"><?php echo $clist['name'];?></a></span>
												<ul class="nested <?php (in_array($clist['id'], $parent_ids)) ? ' active' : '';?>" id="list_<?php echo $clist['id'];?>" style="margin-left: 10px;">
									...
									<div id="chart-of-accounts-child-board" class="chart-of-accounts-col-9-body-container">
										...
										<div style="position: relative;">
											<table class="list-table table table-hover not-clickable chart-of-accounts-col-9-body-container-table">
												<tbody>
													<?php 
														foreach($category_list->result_array() as $clist){ 
										...
										<?php }else{ ?>
										<div class="py-2"><h5 class="text-dark">This Account is Empty</h5></div>
										<?php } ?>
										<a type="button" onclick="addAccount(this)" data-folderid="<?php echo $parent_detail['id']; ?>" class="add-account font-weight-bold">
...
<script>
	...
	function addAccount(identifier) {
		...
		if(folderid !== ''){
			$.ajax({
				type: "post",
				url: "<?php echo base_url('admin/cost-center/add-sub-account-form');?>",
	...
	function editAccount(identifier) {
		...
		let folderid = <?php echo $parent_detail['id'];?>;
		if(id !== ''){
			$.ajax({
				type: "post",
				url: "<?php echo base_url('admin/cost-center/edit-sub-account-form');?>",
				data: {'id': id,'parent_id': folderid},
				dataType: "json",
				success: function (response) {
					//console.log(response);
					if(response.type == 'success'){
	...
	function quickSidebar(identifier){
		...
		if(id > 0){
			$.ajax({
				type: "post",
				url: "<?php echo base_url();?>admin/cost-center/get-sidebar-cat",
				data: {'id': id},
				beforeSend: function() {
