            ...
            <div class="col-sm-6">
                <div class="page-title">
                    ...
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">Vehicle Allotment/Unallotment</li>
                    </ol>
                </div>
            </div>
            <div class="col-sm-6">
                <div class="float-end d-sm-block">
                    <button type="button" class="btn btn-custom-success btn-sm pull-right" title="Add" id="addVehicleAllotmentBtn"><i class="fa fa-plus"></i> Add Approval Cycle</button>
                </div>
								...
								<tr>
									...
									<td class="p-0">
										<table class="table custom-hover-table" cellpadding="5" cellspacing="0">
											...
											<tbody>
												<?php 
												if($vehicle_allotment_approvals){
												foreach ($vehicle_allotment_approvals as $result) {
													...
													<tr>
														...
														<td align="right">
															<span class="delete-icon">
																<button class="btn btn-link text-dark mt-0 p-0 me-2 edit-vehicle-allotment-btn" style="vertical-align: inherit;" data-setting-id="<?= $result->id ?>">
    ...
    <div class="modal-dialog modal-dialog-aside">
		<div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title mt-0" id="leaveModalLabel">New Vehicle Allotment Approval Cycles</h5>
...
<div class="modal fade fixed-left employeeSelectionModal" aria-labelledby="#selectEmployeeModalLabel" style="display: none;" aria-hidden="true">
    <div class="modal-dialog modal-dialog-aside">
		<div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title mt-0" id="selectEmployeeModalLabel">Vehicle Allotment</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
...
$('#addVehicleAllotmentBtn').on('click', function() {
    // Make the AJAX request
    $.ajax({
        url: '<?php echo base_url('admin/hr-module/request/vehicle-allotment/add-form'); ?>',
        method: 'GET',
        success: function(responseHtml) {
			...
			//initializeSortable();
			$('#leaveModalLabel').html('Add Vehicle Allotment Approval Cycles');
			var footerHtml = `<div class="row">
					<div class="col-md-12">
						<button form="addVehicleAllotmentForm" type="submit" class="btn btn-success btn-md">Save</button>
...
$('.edit-vehicle-allotment-btn').on('click', function() {
	var id = $(this).data('setting-id');
    // Make the AJAX request
    $.ajax({
        url: '<?php echo base_url('admin/hr-module/request/vehicle-allotment/edit-form'); ?>',
        method: 'GET',
        ...
        success: function(responseHtml) {
			...
			$('.leave-modal-body').html(responseHtml);
			$('.select2').select2({dropdownCssClass: 'custom-dropdown'});
			$('#leaveModalLabel').html('Edit Vehicle Allotment Approval Cycles');
			var footerHtml = `<div class="row">
					<div class="col-md-12">
...
$(document).on('click', '.delete-btn2', function() {
	...
	Swal.fire({
		...
		if (result.isConfirmed) {
			$.ajax({
				url: '<?php echo base_url('admin/hr-module/request/vehicle-allotment/delete'); ?>',
				method: 'POST',
				data: { vehicle_allotment_id: requestId },
				dataType: 'json',
