		...
		<div class="row align-items-center">
			<div class="col-sm-6">
				<div class="page-title">
					<h4>Hunger Rider's Adjustment Wallet Report</h4>
					<ol class="breadcrumb m-0">
						<li class="breadcrumb-item"><a href="<?php echo base_url('admin'); ?>">Dashboard</a></li>
						<li class="breadcrumb-item"><a href="<?php echo base_url('admin/logistic-management/hunger/wallet-report'); ?>">Rider's Adjustment Wallet Report</a></li>
			...
			<div class="col-sm-6">
				<div class="float-end d-sm-block">
					<?php if(check_action_permission(get_user_role(), 'hunger_wallet_transaction', 'delete')):?>
					...
					<?php endif; if(check_action_permission(get_user_role(), 'hunger_wallet_transaction', 'import_file')):?>
					<a class="btn btn-custom-white btn-sm pull-right ms-2" title="Import Wallet Report" href="javascript:;" data-bs-toggle="modal" data-bs-target=".bulkImportModal"><i class="ti-import"></i> Import Wallet Report</a>
					<?php endif;?>
				</div>
		...
		<div class="row">
			<div class="col-12">
				<div class="card">
					...
					<div class="card-body">
						<form action="<?php echo base_url('admin/logistic-management/hunger/wallet-report'); ?>" method="get" id="filter_form">
							<div class="row">
							...
							<div class="row mt-2">
								...
								<div class="col-lg-6 col-md-6 col-sm-12">
									...
									<a href="<?php echo base_url('admin/logistic-management/hunger/wallet-report'); ?>" class="btn btn-danger btn-md float-end">Reset Filter</a>
								</div>
							</div>
						</form>
					</div>
				...
				<div class="card">
					<div class="card-body">
						<form id="myform" name="myform" method="post" action="">
							<table id="walletTable" class="table table-striped table-bordered jambo_table bulk_action" style="width:100%">
								<thead>
									<tr>
										<th>#</th>
										<th>S.No.</th>
	...
	<div class="modal-dialog modal-dialog-centered" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title px-3" id="bulkImportModalLabel">Rider's Adjustment Wallet Report <b>(Hunger)</b></h5>
				...
				<form id="delivery_bulk_form" method="post" enctype="multipart/form-data" autocomplete="off">
					<div class="row px-3">
						<div class="col-md-12 col-sm-12 mb-3 form-group">
							<label for="attachment">Rider's Adjustment Wallet File <span class="text-secondary">(Supported file format is: .xls .xlsx)</span></label>
						...
						<div class="col-md-12">
							...
							<p style="text-align: center;"><a type="button" href="<?php echo base_url('admin_assets/samples/riders_adjustment_wallet_sample.xlsx'); ?>" class="btn bt btn-link text-success" target="_blank" download>Download sample file</a></p>
						</div>
...
function initializeDataTable() {
	...
	$('#walletTable').dataTable({
		...
		"searching": false,
		"ajax":{
			url:"<?php echo base_url();?>admin/logistic-management/hunger/wallet-report-list?keyword=<?php echo $this->input->get('keyword')?>&rider_id=<?php echo $this->input->get('rider_id')?>&start_date=<?php echo $this->input->get('start_date')?>&end_date=<?php echo $this->input->get('end_date')?>",
			type:"POST"
		},
		"columnDefs":[
			{
...
function deleteAction() {
	...
	if(inputCount > 0){
		if (confirm("Do you want to delete selected items?") == true) {
			changeActionAndSubmit('admin/logistic-management/hunger/wallet-delete');
		} else {
...
$(document).ready(function() {
	$("body").on("submit", "#delivery_bulk_form", function(e) {
		...
		$.ajax({
			type: 'POST',
			url: "<?php echo base_url('admin/logistic-management/hunger/wallet-import') ?>",
			data: data,
			//dataType: 'json',
			contentType: false,
			cache: false,
			...
			success: function(response) {
				...
				if (jsonResponse.error_message) {
					...
					if (jsonResponse.duplicate_rows) {
						tableHtml += '<h6>Duplicate Rows</h6><table class="table border-danger"><tr><th>Rider ID</th><th>Amount</th><th>Date</th></tr>';
