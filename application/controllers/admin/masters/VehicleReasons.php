...
class VehicleReasons extends CI_Controller
{
	...
	{
		if ($this->action && !check_action_permission(get_user_role(), 'vehiclesstatusreasons', $this->action)) {
		...
		return $this->load->view('admin/masters/vehicle_reason/list', $data);
	}
	...
	public function get_list()
	{
		...
		foreach ($fetch_data as $brand) {
			...
			$sub_array[] = check_action_permission(get_user_role(), 'vehiclesstatusreasons', 'edit') ? '<button type="button" class="btn btn-outline-secondary btn-custom-light btn-sm edit" data-toggle="tooltip" title="Edit" onclick="editModal(' . $brand->id . ')"><i class="mdi mdi-pencil font-size-18"></i></button>' : '';
			$data[] = $sub_array;
		}
		$output = array(
	...
	public function add()
	{
		...
		$this->load->view('admin/masters/vehicle_reason/components/add_modal');
	}
	...
	public function edit()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($query->num_rows() > 0) {
				...
				$output_data = $this->load->view('admin/masters/vehicle_reason/components/edit_modal', $data, TRUE);
	...
	public function save()
	{
		...
		redirect('admin/master/vehicle-reason');
	}
