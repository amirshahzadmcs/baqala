...
class Transfer_model extends CI_Model{
	...
	function make_query() {
		$sql = "SELECT la.* FROM request_approval la WHERE la.approval_types = 'TransferRequest'";
		return $sql;
	}
    ...
    function get_all_data(){
	   ...
	   $this->db->where('approval_types', 'TransferRequest');
	...
	public function get_all_employees() {
		// Check if any TransferRequest has `is_applicable_to_all = yes`
		$this->db->select('is_applicable_to_all, employees_ids');
		$this->db->where('approval_types', 'TransferRequest');
		$query = $this->db->get('request_approval');
		$transferRequests = $query->result();

		$excludeEmployees = []; // Array to store employees to exclude

		foreach ($transferRequests as $transferRequest) {
			if ($transferRequest->is_applicable_to_all === 'yes') {
				// If `is_applicable_to_all = yes`, return an empty array
				return [];
			} elseif ($transferRequest->is_applicable_to_all === 'no' && !empty($transferRequest->employees_ids)) {
				// If `is_applicable_to_all = no`, collect employees_ids to exclude
				$excludeEmployees = array_merge($excludeEmployees, json_decode($transferRequest->employees_ids, true));
			}
		}
