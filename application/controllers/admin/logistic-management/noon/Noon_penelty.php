...
class Noon_penelty extends CI_Controller
{
	...
	public function __construct()
	{
		...
		// Load Model
		$this->load->model('admin/logistic-management/noon/Daily_penelty_model', 'noon_model');
	...
	{
		if (!check_action_permission(get_user_role(), 'noon_daily_penelty', $this->action)) {
		...
		$this->load->view("admin/logistic-management/noon/panelty/index", $data);
	}
	...
	public function ajax_list()
	{
		...
		foreach ($fetch_data['data'] as $item) {
			...
			foreach ($fetch_data['visible_columns'] as $column) {
				$value = $item[$column] ?? '';
				if ($column == 'full_name' || $column == 'order_number') {
					$sub_array[] = '<div class="text-start">' . htmlspecialchars($value) . '</div>';
				} elseif (in_array($column, ['penelty_date', 'created_at', 'updated_at']) && !empty($value)) {
	...
	public function import_file()
	{
		...
		try {
			...
			foreach ($sheet_data as $index => $val) {
				...
				if (!$isDuplicate) {
					$id_number = $this->db->escape_str($val[4]);
					$emp_id = $this->getRiderEmpIdForDay($id_number, $date_local);

					$batch_data[] = [
						"penelty_date" => $date_local,
						"emp_id" => $emp_id,
						"vendor_name" => $val[0],
						"date" => $val[1],
						"city" => $val[2],
						"default_fleet" => trim($val[3]),
						"da_id" => $val[4],
						"payout_type" => $val[5],
						"delivered" => $val[6],
						"total_rejections" => $val[7],
						"rejection_penalty" => $val[8],
						"order_number" => json_encode(array_slice($val, 9)),
						"added_by" => $addedBy,
			...
			if (!empty($duplicate_rows) || !empty($malformed_rows)) {
				$json = [
					'error_message' => count($duplicate_rows) . ' duplicate(s), ' . count($malformed_rows) . ' malformed row(s). <span class="text-success">Rest inserted successfully.</span>',
					'duplicate_rows' => $duplicate_rows,
					'malformed_rows' => $malformed_rows
				];
			} else {
				$json = [
					'success_message' => 'All entries imported successfully.',
				];
			}
		} catch (Exception $e) {
	...
	{
		$result = $this->db->insert_batch('noon_daily_penelty', $batch_data);
	...
	public function print_panelty_report()
	{
		if (!check_action_permission(get_user_role(), 'noon_daily_penelty', $this->action)) {
			redirect('admin/unauthorized-request');
		}
		...
		//echo '<pre>' . print_r($data['report_data'], true) . '</pre>';exit();
		...
		$pdf->SetTitle('Noon Daily Panelty Report');
		$pdf->SetSubject('Noon Daily Panelty Report');
		$pdf->SetKeywords('Maha Al Fala, PDF, Noon Daily Panelty Report');

		// print_r($data);exit();
		// remove default header/footer
		$pdf->setPrintHeader(true);
		$pdf->SetPrintFooter(true);
		$htmlHeader = $this->load->view('admin/logistic-management/noon/panelty/print/header', $data, true);
		$htmlHeader2 = $this->load->view('admin/logistic-management/noon/panelty/print/header', $data, true);
		$pdf->setHtmlHeader($htmlHeader);
		$pdf->setHtmlHeader2($htmlHeader2);

		$lastFooter = $this->load->view('admin/logistic-management/noon/panelty/print/footer', $data, true);
		...
		// Arabic and English content
		$htmlcontent = $this->load->view('admin/logistic-management/noon/panelty/print/report', $data, true);
		$pdf->WriteHTML($htmlcontent, true, 0, true, 0);
		//Close and output PDF document
		$filename = 'BS-Noon-Panelty-Report';
	...
	public function export_panelty_report()
	{
		if (!check_action_permission(get_user_role(), 'noon_daily_penelty', 'print_panelty_report')) {
			redirect('admin/unauthorized-request');
		}
		$this->form_validation->set_rules('column_type', 'Column Type', 'required');
		$this->form_validation->set_rules('file_format', 'Format Type', 'required');
		...
		$sheet->setCellValue('A1', 'Noon Daily Penalty Report');
		...
		foreach ($reportData['data'] as $detail) {
			...
			foreach ($reportData['available_columns'] as $column) {
				...
				// Format date fields
				if (in_array($cleanedColumn, ['panelty_date', 'created_at', 'updated_at']) && !empty($value)) {
	...
	public function delete()
	{
		...
		redirect('admin/logistic-management/noon-panelty/list');
	}
}
