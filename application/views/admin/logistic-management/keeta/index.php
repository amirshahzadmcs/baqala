		...
		<div class="row align-items-center">
			<div class="col-sm-6">
				<div class="page-title">
					<h4>Keeta Daily Order</h4>
					<ol class="breadcrumb m-0">
						<li class="breadcrumb-item"><a href="<?php echo base_url('admin'); ?>">Dashboard</a></li>
						<li class="breadcrumb-item"><a href="<?php echo base_url('admin/logistic-management/keeta/index'); ?>">Keeta Orders</a></li>
			...
			<div class="col-sm-6">
				<div class="float-end d-sm-block">
					<?php if (check_action_permission(get_user_role(), 'keeta_daily_performance', 'delete')): ?>
					...
					<?php endif;
					if (check_action_permission(get_user_role(), 'keeta_daily_performance', 'import_file')): ?>
		...
		<div class="row">
			<div class="col-12">
				<div class="card">
					...
					<div class="card-body">
						<form action="<?php echo base_url('admin/logistic-management/keeta/index'); ?>" method="get" id="filter_form">
							<div class="row">
								...
								<div class="col-lg-4 col-md-4 col-sm-12">
									<div class="form-group mb-2">
										...
										<input type="search" name="courier_id" placeholder="Search Courier ID Number" value="<?php echo $this->input->get('courier_id') ? $this->input->get('courier_id') : ''; ?>" autocomplete="off" class="form-control">
									</div>
								</div>
							...
							<div class="row mt-2">
								...
								<div class="col-lg-6 col-md-6 col-sm-12">
									...
									<a href="<?php echo base_url('admin/logistic-management/keeta/index'); ?>" class="btn btn-danger btn-md float-end">Reset Filter</a>
								</div>
							</div>
						</form>
					</div>
							...
							<table id="empTable" class="table table-striped table-bordered jambo_table bulk_action" style="width:100%">
								<thead>
									<tr>
										...
										<th>Date</th>
										<th>Courier ID</th>
										<th>Rider/Driver Name</th>
										<th>Vehicle Type</th>
										<th>Accepted Tasks</th>
										<th>Delivered Tasks</th>
										<th>Cancelled Tasks</th>
										<th>Rejected Tasks</th>
										<th>Rejected Tasks (Courier)</th>
										<th>Rejected Tasks (Auto)</th>
										<th>Courier App Online Time</th>
										<th>On-time Rate (D)</th>
										<th>Avg Delv. Time of Delivd. Orders</th>
										<th>Delivd. Orders Prop. (Over 55min)</th>
										<th>Created At</th>
										<!-- <th>Tools</th> -->
									</tr>
								</thead>
	...
	<div class="modal-dialog modal-dialog-centered" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title px-3" id="bulkImportModalLabel">Rider Daily Performance <b>(Keeta)</b></h5>
				...
				<form id="delivery_bulk_form" method="post" enctype="multipart/form-data" autocomplete="off">
					<div class="row px-3">
						...
						<div class="col-md-12">
							...
							<p style="text-align: center;"><a type="button" href="<?php echo base_url('admin_assets/samples/Keeta_Sample_File.xlsx'); ?>" class="btn bt btn-link text-success" target="_blank" download>Download sample file</a></p>
						</div>
	...
	$(document).ready(function() {
		...
		function initializeDataTable() {
			...
			$('#empTable').dataTable({
				...
				"fixedHeader": true,
				"ajax": {
					url: "<?php echo base_url(); ?>admin/logistic-management/keeta/ajax-list?keyword=<?php echo $this->input->get('keyword') ?>&courier_id=<?php echo $this->input->get('courier_id') ?>&start_date=<?php echo $this->input->get('start_date') ?>&end_date=<?php echo $this->input->get('end_date') ?>",
					type: "POST"
				},
				"columnDefs": [{
		...
		$("body").on("submit", "#delivery_bulk_form", function(e) {
			...
			$.ajax({
				type: 'POST',
				url: "<?php echo base_url('admin/logistic-management/keeta/import') ?>",
				data: data,
				//dataType: 'json',
				contentType: false,
				cache: false,
				...
				success: function(response) {
					...
					if (jsonResponse.error_message) {
						...
						if (jsonResponse.duplicate_rows) {
							tableHtml += '<h6>Duplicate Rows</h6><table class="table border-danger"><tr><th>Date</th><th>Courier ID</th><th>Courier First Name</th></tr>';
	...
	function deleteAction() {
		...
		if (inputCount > 0) {
			if (confirm("Do you want to delete selected items?") == true) {
				changeActionAndSubmit('admin/employed-rider/keeta/delete');
			} else {
