...
class Notice_warning extends CI_Controller {

	public function __construct() {
		parent::__construct();									
		if($this->admin->isLogged()){
			$this->load->model('admin/requests/Warning_model', 'warning_model');
	...
	public function index()
	{
		...
		$data['warning_approvals'] = $this->warning_model->get_list();
		//dd($data['loan_approvals']);
		$this->load->view('admin/requests/notices_warnings/index',$data);
	}

	public function add_form() {
        $this->load->view('admin/requests/notices_warnings/partials/add_approval');
    }

	public function get_employee_view() {
		$data['employees'] = $this->warning_model->get_all_employees();
        $this->load->view('admin/requests/notices_warnings/partials/employee_list', $data);
    }

	public function edit_form() {
		$id = $this->input->get('id');
		$data['warning_approval'] = $this->warning_model->getRequestById($id);
		//dd($data['loan_requests']);
        $this->load->view('admin/requests/notices_warnings/partials/edit_approval', $data);
    }
	...
	public function save_approval() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$insertData = [
				'name' => $data['name'],
				'approval_types' => 'NoticesAndWarning',
			...
			if ($is_applicable_to_all === 'yes') {
				$this->db->where('approval_types', 'NoticesAndWarning');
			...
			if ($this->dl_model->insertApporval($insertData)) {
				echo json_encode(['status' => 'success', 'message' => 'Notice approval cycles added successfully.']);
				return;
			} else {
				echo json_encode(['status' => 'error', 'message' => 'Failed to add notice approval cycles.']);
				return;
			}
		}
	}	
	...
	public function update_approval() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data = $this->input->post();
			$selectedEmployees = json_decode($data['selectedEmployees'], true);
			$dlId = $data['request_id'];
			$dlDetail = $this->warning_model->getRequestById($dlId);
			...
			// Update existing NoticesAndWarning records if 'is_applicable_to_all' is 'yes'
			if ($is_applicable_to_all === 'yes') {
				$this->db->query("UPDATE `request_approval` 
						SET `is_applicable_to_all` = 'no', 
							`employees_ids` = JSON_ARRAY() 
						WHERE `approval_types` = 'NoticesAndWarning'");
			...
			if ($this->warning_model->updateApporval($dlId, $dlData)) {
				echo json_encode(['status' => 'success', 'message' => 'Notices approval cycles updated successfully.']);
				return;
			} else {
				echo json_encode(['status' => 'error', 'message' => 'Failed to update notice approval cycles.']);
				return;
			}
		}
	}	
	...
	public function delete_approval() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$result = $this->warning_model->deleteApproval($request_id);
			if ($result) {
				echo json_encode(['status' => 'success', 'message' => 'Notices deleted successfully.']);
			} else {
				echo json_encode(['status' => 'error', 'message' => 'Error deleting notices.']);
			}
		}
    }
