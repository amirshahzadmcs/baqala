		...
		<div class="row align-items-center">
			<div class="col-sm-6">
				<div class="page-title">
					<h4>Hunger <?= ((isset($summaryData['summary_month'])) ? date('F Y', strtotime($summaryData['summary_month'])) : ''); ?> Summary</h4>
			...
			<div class="col-sm-6">
				<div class="float-end d-sm-block">
					<a class="btn btn-sm btn-custom-white pull-right" title="Back" href="<?php echo base_url('admin/logistic-management/invoices/hunger/list'); ?>"><i class="fa fa-reply"></i> Back</a>
					<div class="btn-group ms-2 float-end">
						...
						<div class="dropdown-menu dropdown-menu-end">
							...
							<a href="<?php echo base_url('admin/logistic-management/invoices/hunger/export-monthly-data/'.$summaryData['id']);?>" class="dropdown-item" title="Monthly Data" target="_blank">Export Monthly Data</a>
						</div>
					</div>
				</div>
				...
				<div class="card">
					<div class="card-body">
						<div class="row">
							<!--
							<div class="col-md-6">
								<h4><b><?= ((isset($summaryData['summary_month'])) ? date('F Y', strtotime($summaryData['summary_month'])) : ''); ?> Summary</b></h4>
								<p class="mb-0">
									<?= ((isset($summaryData['summary_month'])) ? date('F Y', strtotime($summaryData['summary_month'])) : ''); ?>,
									<?= ((isset($summaryData['from_date'])) ? date('M d', strtotime($summaryData['from_date'])) : ''); ?>
									to
									<?= ((isset($summaryData['to_date'])) ? date('M d', strtotime($summaryData['to_date'])) : ''); ?>
								</p>
							</div>
							<div class="col-md-6">
											...
											<?php
												if (!empty($visibleTableColumns)) {
													foreach ($visibleTableColumns as $column) {
														...
														echo "<th>" . ucwords(str_replace('_', ' ', $cleanedHeader)) . "</th>";
													}
												}
											?>
										</tr>
...
$(document).ready(function() {
    ...
    function loadFilteredData(page = 1) {
        ...
        // AJAX Request
        $.ajax({
            url: "<?= base_url('admin/logistic-management/invoices/hunger/detail/' . $summaryData['id']) ?>",
            type: 'GET',
            ...
            success: function(response) {
				...
				if (availableColumns.length) {
					...
					availableColumns.forEach(column => {
						const cleanedHeader = column.replace('mhms.', '').replace('me.', '');
...
function getColumnForm() {
	$.ajax({
		...
		method: 'GET',
		data: { request_type: 'hunger_monthly_summary' },
		success: function(response) {
