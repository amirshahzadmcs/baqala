...
class Hunger_sales extends CI_Controller
{
	...
	public function __construct()
	{
		...
		// Load Model
		$this->load->model('admin/logistic-management/aggregators/hunger/Hunger_sales_model', 'hunger_sales_model');
	...
	{
		if ($this->action && !check_action_permission(get_user_role(), 'sales_data', $this->action)) {
		...
		$data['hunger_summary'] = $this->hunger_sales_model->getSummary();
		$this->load->view("admin/logistic-management/aggregators/hunger_sales/index", $data);
	}
	...
	public function import_file()
	{
		...
		try {
			...
			$existing_records = $this->hunger_sales_model->checkDuplicateInBulk($summary_date);
			if (!empty($existing_records)) {
				...
				$this->db->insert('maha_hunger_sales_data', $data);
				...
				foreach ($sheet_data as $val) {
					...
					if (!empty($val[0])) { // Replace with necessary column checks
						$batch_data[] = [
							...
							"rider_id" => $val[0],
							"contract_name" => $val[1],
							"completed_orders" => $val[2],
							"basic_payment" => $val[3],
							"acceptance_rate_penalties" => $val[4],
							"contact_rate_penalties" => $val[5],
							"stacking_deduction" => $val[6],
							"declined_penalties_day_logic" => $val[7],
							"google_capped_after" => $val[8],
							"rider_scoring" => $val[9],
							"rider_basic" => $val[10],
							"monthly_balance" => $val[11],
							"created_at" => $this->datetime,
							"updated_at" => $this->datetime,
						];
	...
	{
		$result = $this->db->insert_batch('maha_hunger_sales_summary', $batch_data);
	...
	public function detail($id)
	{
		...
		if ($id) {
			...
			$config['base_url'] = base_url("admin/logistic-management/invoices/hunger-sales/detail/$id");
			$config['suffix'] = "&search=" . urlencode($search) . "&per_page=" . $perPage;
			$config['first_url'] = $config['base_url'] . '?page=1' . $config['suffix'];
			$config['page_query_string'] = TRUE;
			$config['query_string_segment'] = 'page';
			$config['total_rows'] = $this->hunger_sales_model->summaryDetailCount($id, $search);
			...
			$summaryData = $this->hunger_sales_model->get_detail($id);
			$summaryDetails = $this->hunger_sales_model->summaryDetail($id, $search, $perPage, $page);
			//dd($summaryDetails);

			if ($isAjax) {
				$html = $this->load->view('admin/logistic-management/aggregators/hunger_sales/components/summary-data', ['summaryDetails' => $summaryDetails['data'], 'start' => $data['start'], 'availableTableColumns' => $summaryDetails['visible_columns'], 'visibleTableColumns' => $summaryDetails['visible_columns']], TRUE);
				echo json_encode([
					'html' => $html,
			...
			//dd($data);
			$this->load->view('admin/logistic-management/aggregators/hunger_sales/detail', $data);
		} else {
			$this->session->set_userdata('msg_info', "2--Invalid request!");
			redirect('admin/logistic-management/invoices/hunger-sales/list');
		}
	}
	...
	public function print_sales_data()
	{
		...
		$data['report'] = $this->hunger_sales_model->get_detail($id);
		$data['report_data'] = $this->hunger_sales_model->printSalesReport($id, $search, $column_type, $file_format);
		...
		$pdf->SetTitle('Hunger Monthly Sales Report -' . $data['month_of']);
		$pdf->SetSubject('Hunger Monthly Sales Report -' . $data['month_of']);
		$pdf->SetKeywords('Maha Al Fala, PDF, Hunger Monthly Sales Report');

		// print_r($data);exit();
		// remove default header/footer
		$pdf->setPrintHeader(true);
		$pdf->SetPrintFooter(true);
		$htmlHeader = $this->load->view('admin/logistic-management/aggregators/hunger_sales/print/header', $data, true);
		$htmlHeader2 = $this->load->view('admin/logistic-management/aggregators/hunger_sales/print/header', $data, true);
		$pdf->setHtmlHeader($htmlHeader);
		$pdf->setHtmlHeader2($htmlHeader2);

		$lastFooter = $this->load->view('admin/logistic-management/aggregators/hunger_sales/print/footer', $data, true);
		...
		// Arabic and English content
		$htmlcontent = $this->load->view('admin/logistic-management/aggregators/hunger_sales/print/report', $data, true);
		$pdf->WriteHTML($htmlcontent, true, 0, true, 0);
		//Close and output PDF document
		$pdf->Output('BS-Hunger-Station-Sales-Report-' . $data['month_of'] . '.pdf', 'I');
	...
	public function export_sales_data()
	{
		...
		$filename = "Hunger-Sales-Report-" . $data['month_of'] . ".xlsx";

		// Calculate Summary Data
		$uniqueRiders = [];
		$completedDeliveries = 0;
		$totalBasicPayments = 0;

		foreach ($report_data as $detail) {
			if (!empty($detail['rider_id'])) {
				$uniqueRiders[$detail['rider_id']] = true;
			}
			$completedDeliveries += (int) ($detail['completed_orders'] ?? 0);
			$totalBasicPayments += (int) ($detail['basic_payment'] ?? 0);
		}
		...
		$sheet->setCellValue('A1', 'Hunger Monthly Sales Report - ' . $data['month_of']);
		...
		$sheet->setCellValue('G2', "Total Basic Payments")->setCellValue('H2', $totalBasicPayments);
		...
		foreach ($data['report_data']['available_columns'] as $column) {
			$cleanedHeader = strtoupper(str_replace(['mhss.', 'me.'], '', $column));
