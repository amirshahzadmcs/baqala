...
class Transfer extends CI_Controller
{
	...
	{
		if ($this->action && !check_action_permission(get_user_role(), 'employee_transfer', $this->action)) {
	...
	public function get_list()
	{
		...
		foreach ($transfer_list as $key => $value) {
			...
			if (check_action_permission(get_user_role(), 'employee_transfer', 'print_batch_detail')) {
			$printButtons .= '<a class="dropdown-item" href="' . base_url('admin/hr/employee-transfer/export-pdf/' . $value['batch_no']) . '" target="_blank">
				Export Transfer [PDF]
			</a>';
			}
			if (check_action_permission(get_user_role(), 'employee_transfer', 'export_batch_excel')) {
			...
			if (check_action_permission(get_user_role(), 'employee_transfer', 'view_batch_detail')) {
