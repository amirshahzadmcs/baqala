...
<style>
	...
	.result-tr .a-CL {
		background-color: #aaf038;
	}

	.result-tr .a-ML {
		background-color: #38f0b5;
	}
	...
	.result-tr .a-H {
		background-color: #385af0;
		color: #fff;
	}

	.result-tr .a-TR {
		background-color: #9138f0;
		color: #fff;
	}
	...
	<div class="page-content-wrapper">
		<div class="row">
			...
			<div class="col-12">
				<div class="card">
					<div class="card-body">
						<form action="<?php echo base_url('admin/attendance/list') ?>" method="get">
							<div class="row">
										...
										<tr class="text-white h6" style="background-color: #026902cc!important">
											<td colspan="<?php echo (3 + ($total_days) + 2); ?>" align="center" style="padding: 18px;">
												<table style="position: absolute;width: 100%;text-align: center;">
													<tr>
														<td style="line-height: 0px;">Attendance of <?php echo $attendance_month; ?></td>
													</tr>
												</table>
											</td>
										</tr>
										...
										<?php if (!empty($attend_results) && count($attend_results) > 0) { ?>
											...
											?>
												<tr class="result-tr">
													...
													<?php if (count($summary['order_info']) > 0) {
														foreach ($summary['order_info'] as $att_info) { ?>
															<?php
															...
															<?php } else {
															?>
																<td align="center" class="<?= 'a-' . $att_info['attend_type']; ?>"><?= $att_info['attend_type']; ?></td>
																<?php //($att_info['total_orders']) ? $total_present[$result_count]++ : $total_absent[$result_count]++;
																?>
													<?php }
														}
													} ?>
													<td align="center" class="tpre"><?php //$sumPresent += $total_present[$result_count]; echo $total_present[$result_count];
																					?></td>
													<td align="center" class="tabs"><?php //$sumAbsent += $total_absent[$result_count]; echo $total_absent[$result_count];
																					?></td>
												</tr>
											<?php $result_count++;
											} ?>
											<tr>
												...
												<?php foreach ($summary['order_info'] as $summ) { ?>
													<td><b><?php //echo $summ->present_riders; 
															?></b></td>
												<?php } ?>
