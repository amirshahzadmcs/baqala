		...
		<div class="row align-items-center">
			<div class="col-sm-6">
				<div class="page-title">
					...
					<ol class="breadcrumb m-0">
						...
						<li class="breadcrumb-item"><a href="<?php echo base_url('admin/spare-parts/po/list');?>">SP Purchase Order</a></li>
			...
			<div class="col-sm-6">
				<div class="float-end d-sm-block">
					<a class="btn btn-sm btn-custom-white pull-right me-1" title="Back" href="<?php echo base_url('admin/spare-parts/po/list');?>"><i class="fa fa-reply"></i> Back</a>
 	...
 	<div class="page-content-wrapper">
 		<div class="row">
 			 <div class="col-12">
 				 <div class="card">
 					<div class="card-body">
					 	<form id="demo-form2" method="post" action="<?php echo base_url('admin/spare-parts/po/update-international')?>" class="form-label-left" data-toggle="validator" role="form">
								...
								<tr>
									<td colspan="3">
										<div class="search-bar-container">
											<h5>Search and add spare parts to SP PO</h5>
										...
										<table class="table table-striped jambo_table table-bordered mb-0" id="item_table" width="100%" border="1" cellspacing="0" cellpadding="5">
											<thead>
												<tr>
													...
													<td valign="top" bgcolor="#CCCCCC" style="width: 8%;"><strong>Qty</strong></td>
													<td valign="top" bgcolor="#CCCCCC" style="width: 8%;"><strong>FCY</strong></td>
											...
											<tbody>
												...
												<tr id="item-row<?php echo $item_row;?>">
													<td valign="middle" align="center"><button type="button" onclick="remove_item(<?php echo $item_row;?>)" data-toggle="tooltip" title="Remove" class="btn btn-danger btn-sm"><i class="fa fa-minus-circle"></i></button></td>
													...
													<td valign="middle"><input type="number" min="0" name="qty[]" value="<?php echo $product->qty; ?>" class="form-control item_unit" required="required" /></td>
													<td valign="middle"><input type="text" min="0" name="fcy[]" value="<?php echo $product->fcy; ?>" class="form-control item_fcy" required="required" /></td>
													<td valign="middle"><input type="text" min="0" name="price[]" value="<?php echo $product->price; ?>" class="form-control unit_price" required="required" /></td>
													<td valign="middle"><input type="text" min="0" name="line_total[]" value="<?php echo $product->total; ?>" class="form-control item_total" required="required" /></td>
												</tr>
												...
												<tr>
													...
													<td style="vertical-align: top; width: 40%;padding: 0;">
														<table class="table table-bordered" id="tab_logic_total" width="100%" cellspacing="0" cellpadding="5" style="border: 1px solid #ddd;">
															...
															<tr>
																<td style="text-align: left;">TOTAL SAR</td>
																<td class="amount"><input type="text" name="total" id="total_sar" value="<?php echo $order->total_cost;?>" class="form-control" readonly /></td>
															</tr>
															<tr>
																<td valign="middle" style="text-align: left; width: 68%;">TOTAL FCY (VAT INC.):</td>
																<td valign="middle" class="total-amount"><input type="text" name="total_fcy" value="<?php echo $order->total_fcy;?>" id="total_amount" class="form-control total" readonly /></td>
															</tr>
														</table>
													</td>
												</tr>
...
<script>
	...
	function addItem(id,part_name_en,part_name_ar,item_code,vehicle_make,vehicle_model){
		...
		html += ' <td class="text-left"><textarea name="spare_part_name[]" class="form-control" readonly="readonly" required="required" style="height: 38px;">' + part_name_en + '</textarea></td>';
		html += '<td class="text-left"><input type="number" min="0" name="qty[]" value="0" class="form-control item_unit" required="required" /></td>';
		html += '<td class="text-left"><input type="text" min="0" name="fcy[]" value="0" class="form-control item_fcy" required="required" /></td>';
		html += '<td class="text-left"><input type="text" min="0" name="price[]" value="0" class="form-control unit_price" required="required" /></td>';
		html += '<td class="text-left"><input type="text" min="0.1" name="line_total[]" value="0" class="form-control item_total" required="required" /></td>';
		html += '</tr>';
...
<script>
	...
	function calc() {
		$('#item_table tbody tr').each(function(item_row, element) {
			...
			if (html != '') {
				...
				var price = $(this).find('.item_fcy').val();
	...
	function calc_total() {
		...
		$('.unit_price').each(function() {
			total_sar += parseFloat($(this).val());
		});
		$('#sub_total').val(total.toFixed(2));
		$('#total_sar').val(total_sar.toFixed(2));
		main_total = total;
