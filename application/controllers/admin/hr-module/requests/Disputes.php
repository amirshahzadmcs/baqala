...
class Disputes extends CI_Controller {
	...
	public function __construct() {
		...
		if($this->admin->isLogged()){
			$this->load->model('admin/requests/Disputes_model', 'dispute_model');
	...
	public function index()
	{
		...
		$data['dispute_approvals'] = $this->dispute_model->get_list();
		//dd($data['dispute_approvals']);
		$this->load->view('admin/requests/disputes/index',$data);
	}

	public function add_form() {
        $this->load->view('admin/requests/disputes/partials/add_approval');
    }

	public function get_employee_view() {
		$data['employees'] = $this->dispute_model->get_all_employees();
        $this->load->view('admin/requests/disputes/partials/employee_list', $data);
    }

	public function edit_form() {
		$id = $this->input->get('id');
		$data['disputes_approval'] = $this->dispute_model->getRequestById($id);
		//dd($data['loan_requests']);
        $this->load->view('admin/requests/disputes/partials/edit_approval', $data);
    }
	...
	public function save_approval() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->dispute_model->insertApporval($insertData)) {
				echo json_encode(['status' => 'success', 'message' => 'Dispute approval cycles added successfully.']);
				return;
			} else {
				echo json_encode(['status' => 'error', 'message' => 'Failed to add dispute approval cycles.']);
				return;
			}
		}
	}	
	...
	public function update_approval() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$dlDetail = $this->dispute_model->getRequestById($dlId);
			...
			// Update existing Disputes records if 'is_applicable_to_all' is 'yes'
			if ($is_applicable_to_all === 'yes') {
				$this->db->query("UPDATE `request_approval` 
						SET `is_applicable_to_all` = 'no', 
							`employees_ids` = JSON_ARRAY() 
						WHERE `approval_types` = 'Disputes'");
			...
			if ($this->dispute_model->updateApporval($dlId, $dlData)) {
				echo json_encode(['status' => 'success', 'message' => 'Dispute approval cycles updated successfully.']);
				return;
			} else {
				echo json_encode(['status' => 'error', 'message' => 'Failed to update dispute approval cycles.']);
				return;
			}
		}
	}	
	...
	public function delete_approval() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$result = $this->dispute_model->deleteApproval($request_id);
			if ($result) {
				echo json_encode(['status' => 'success', 'message' => 'Disputes deleted successfully.']);
			} else {
				echo json_encode(['status' => 'error', 'message' => 'Error deleting disputes.']);
			}
		}
    }
