		...
		<div class="row align-items-center">
			...
			<div class="col-sm-6">
				<div class="float-end d-sm-block">
					...
					-->
					<?php if(check_action_permission(get_user_role(),'3p_logistic_partner','delete')):?>
					<button type="button" class="btn btn-custom-danger btn-sm pull-right me-2" onclick="deleteAction()" data-toggle="tooltip" title="Delete"><i class="fa fa-trash"></i> Delete</button>
					<?php endif; if(check_action_permission(get_user_role(),'3p_logistic_partner','add')):?>
					<a class="btn btn-custom-success btn-sm pull-right" data-toggle="tooltip" title="Add" href="<?php echo base_url('admin/logistic-partner/add')?>"><i class="fa fa-plus"></i> Add</a>
					<?php endif;?>
				</div>
 	...
 	<div class="page-content-wrapper">
		 ...
		 <div class="col-12">
				<div class="card">
					...
					<div class="card-body">
						<form action="<?php echo base_url('admin/logistic-partner/list'); ?>" method="get" id="filter_form">
							<div class="row">
								<div class="col-lg-4 col-md-4 col-sm-12">
									<div class="form-group mb-2">
										<label>Search by Name / Cust. No. / Email Address</label>
										<input type="search" id="keyword" name="keyword" placeholder="Search by Name, Cust. No., Email Address" value="<?php echo $this->input->get('keyword') ? $this->input->get('keyword') : ''; ?>" autocomplete="off" class="form-control">
									</div>
								</div>
								...
								<div class="row">
									<div class="col-lg-4 col-md-4 col-sm-12">
										<div class="form-group mb-2">
											...
											<input type="search" id="vat_no" name="vat_no" placeholder="Search by VAT Number" value="<?php echo $this->input->get('vat_no') ? $this->input->get('vat_no') : ''; ?>" autocomplete="off" class="form-control">
										</div>
									</div>
									...
									<div class="col-lg-4 col-md-4 col-sm-12">
										<div class="form-group mb-2">
											...
											<input type="search" id="cr_no" name="cr_no" placeholder="Search by CR Number" value="<?php echo $this->input->get('cr_no') ? $this->input->get('cr_no') : ''; ?>" autocomplete="off" class="form-control">
										</div>
									</div>
							...
							<div class="row mt-2">
								...
								<div class="col-lg-6 col-md-6 col-sm-12">
									...
									<a href="<?php echo base_url('admin/logistic-partner/list'); ?>" class="btn btn-danger btn-md float-end">Reset Filter</a>
								</div>
							</div>
						</form>
					</div>
 				...
 				<div class="card">
 					<div class="card-body">
					 	<form id="myform" name="myform" method="post" action="">
		 					<table id="partnerTable" class="table table-striped table-bordered jambo_table bulk_action" style="width:100%">
		 						<thead>
		 							<tr>					  
										<th>#</th>
										<th>Customer No.</th>				  
...
$(document).ready(function() {
	$('#partnerTable').dataTable({
		...
		"order":[],
		"ajax":{
				url:"<?php echo base_url();?>admin/logistic-partner/partner-list?keyword=<?php echo $this->input->get('keyword')?>&status=<?php echo $this->input->get('status')?>&application_status=<?php echo $this->input->get('application_status')?>&from=<?php echo $this->input->get('from')?>&to=<?php echo $this->input->get('to')?>&vat_no=<?php echo $this->input->get('vat_no')?>&cr_no=<?php echo $this->input->get('cr_no')?>",
				type:"POST",
...
function deleteAction() {
	...
	if(inputCount > 0){
		if (confirm("Do you want to delete selected item?") == true) {
			changeActionAndSubmit('admin/logistic-partner/delete');
		} else {
...
var EnableStatus = function() {
	var inputCount = $('[name="checklist[]"]:checked').length;
	if(inputCount > 0){
		if (confirm("Do you want to enable selected item?") == true) {
			changeActionAndSubmit('Partner/setStatusEnable');
		} else {
			userPreference = "Action Cancelled!";
		}
	}else{
...
function DisableStatus() {
	var inputCount = $('[name="checklist[]"]:checked').length;
	if(inputCount > 0){
		if (confirm("Do you want to disable selected item?") == true) {
			changeActionAndSubmit('Partner/setStatusDisable');
		} else {
			userPreference = "Action Cancelled!";
		}
	}else{
