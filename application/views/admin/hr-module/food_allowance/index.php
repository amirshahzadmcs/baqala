...
#foodAllownceTable thead tr th, #foodAllownceTable tbody tr td{
	white-space: nowrap;
}
.batch-detail-body {
    overflow-x: scroll;
		...
		<div class="row align-items-center">
			<div class="col-sm-6">
				<div class="page-title">
					<h4>Food Allowance Distribution</h4>
					<ol class="breadcrumb m-0">
						<li class="breadcrumb-item"><a href="<?php echo base_url('admin'); ?>">Dashboard</a></li>
						<li class="breadcrumb-item"><a href="<?php echo base_url('admin/logistic-management/keeta/index'); ?>">Food Allowance Distribution</a></li>
			...
			<div class="col-sm-6">
				<div class="float-end d-sm-block">
					<?php if(check_action_permission(get_user_role(), 'food_allowance_request', 'delete_food_allowance')):?>
					<button type="button" class="btn btn-custom-danger btn-sm pull-right me-2" onclick="deleteAction()" title="Delete"><i class="fa fa-trash me-1"></i> Delete</button>
					<?php endif; if(check_action_permission(get_user_role(), 'food_allowance_request', 'add_allowance_form')):?>
					<button type="button" class="btn btn-custom-success btn-sm pull-right" title="Add Food Allowance Distribution" id="addAllowanceBtn"><i class="fa fa-plus"></i> Add Food Allowance</button>
					<?php endif; ?>
				</div>
	...
	<div class="page-content-wrapper">
		...
		<div class="col-12">
				<div class="card">
					...
					<div class="card-body">
						<form action="<?php echo base_url('admin/hr/food-allowance/index'); ?>" method="get" id="filter_form">
							<div class="row">
								...
								<div class="form-group col-lg-4 col-md-4 col-12 mb-3">
									<label for="date_range">Arrival Date Between:</label>
								...
								<div class="form-group col-lg-4 col-md-4 col-12" style="margin-top: 28px;">
									...
									<a href="<?php echo base_url('admin/hr/food-allowance/index'); ?>" class="btn btn-danger">Reset Filter</a>
								</div>
							</div>

							<?php
								$adv_show = !empty($this->input->get('applied_for')) || !empty($this->input->get('country'));
							?>
								...
								<div class="row">
									...
									<div class="col-lg-4 col-md-4 col-sm-12">
										<div class="form-group mb-2">
											...
											<select name="applied_for" class="form-control select2">
							...
							<div class="row mt-2">
								...
								<div class="col-lg-6 col-md-6 col-sm-12">
									...
									<a href="<?php echo base_url('admin/hr/food-allowance/index'); ?>" class="btn btn-danger float-end">Reset Filter</a>
								</div>
							</div>
							-->
						</form>
				...
				<div class="card">
					<div class="card-body">
						<form id="myform" name="myform" method="post" action="" style="overflow: auto;">
							<table id="foodAllownceTable" class="table table-striped table-bordered jambo_table bulk_action text-center" style="width:100%">
								<thead>
									<tr>
										<th>S.No.</th>
										<th>Batch No.</th>
...
<div class="modal fade fixed-left foodModal" aria-labelledby="#foodModalLabel" style="display: none;" aria-hidden="true">
    <div class="modal-dialog modal-dialog-aside">
		<div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title mt-0" id="foodModalLabel">Food Allowance Distribution Form</h5>
...
<script>
	function initializeDataTable() {
		if ($.fn.DataTable.isDataTable('#foodAllownceTable')) {
			$('#foodAllownceTable').DataTable().destroy();
		}

		$('#foodAllownceTable').dataTable({
			...
			"fixedHeader": true,
			"ajax":{
				url:"<?php echo base_url();?>admin/hr/food-allowance/ajax-list?batch_no=<?php echo $this->input->get('batch_no')?>&start_date=<?php echo $this->input->get('start_date')?>&end_date=<?php echo $this->input->get('end_date')?>",
				type:"POST"
			},
			"columnDefs":[
				{
	...
	$(document).ready(function() {
		...
		$('#addAllowanceBtn').on('click', function() {
			...
			$.ajax({
				url: '<?php echo base_url('admin/hr/food-allowance/add-allowance-form'); ?>',
				method: 'GET',
				success: function(responseHtml) {
					$('#foodModalLabel').html('Food Allowance Distribution Form');
					$('.foodModal').modal('show');
					$('.food-modal-body').html(responseHtml);
					$('.select2').select2({dropdownCssClass: 'custom-dropdown'});
					// Remove the mask if already applied to avoid duplicates
					$('#amount').inputmask('remove');
		
					// Reinitialize the input mask
					$('#amount').inputmask({
						alias: 'numeric',
						digits: 2,
	...
	function viewBatchDetail(batch_no) {
		// Make the AJAX request
		$.ajax({
			url: '<?php echo base_url('admin/hr/food-allowance/view-batch-detail'); ?>',
			method: 'GET',
	...
	function getBatchCv(batch_no) {
		// Make the AJAX request
		$.ajax({
			url: '<?php echo base_url('admin/hr/food-allowance/get-batch-cvs'); ?>',
			method: 'GET',
			data: { batch_no: batch_no },
			success: function(responseHtml) {
				$('#foodModalLabel').html('Food Allowance Distribution Form - '+ batch_no);
				$('.foodModal').modal('show');
				$('.food-modal-body').html(responseHtml);
				var footerHtml = `<div class="row">
						<div class="col-md-12 mb-1">
							<input class="checkbox" type="checkbox" name="confirm_payment" id="paymentConfirmation" required>
							<label class="form-check-label" for="paymentConfirmation" style="vertical-align: top;">Are you sure, want to complete this transaction?</label>
						</div>
						<div class="col-md-12">
							<button form="updateAllowanceForm" type="submit" id="btnUpload" class="btn btn-success btn-md">Save</button>
