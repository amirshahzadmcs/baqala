...
class ShiftController extends CI_Controller
{
	...
	{
		parent::__construct();
		if ($this->admin->isLogged() || $this->teamleader->isLogged()) {
	...
	{
		if ($this->action && !check_action_permission(get_user_role(), 'shift_management', $this->action)) {
		...
		$data['team_leaders'] = $this->db->select('me.id,me.emp_no,me.full_name')
			->join('master_employee me', 'hunger_team.team_leader=me.id', 'left')
			->get('hunger_team')->result();
		$this->load->view('admin/logistic-management/hunger/shift_management/shift_management', $data);
	}
	...
	public function get_riders_shift()
	{
		...
		foreach ($fetch_data as $shift) {
			...
			$sub_array[] = '<div class="d-flex">'
				. (check_action_permission(get_user_role(), 'shift_management', 'getShiftChange') ? '<a class="btn btn-outline-info btn-custom-light btn-sm edit" title="Copy" onclick="changeShift(this)" href="javascript:void(0)" shift_id="' . $shift->id . '" rider_id="' . $shift->rider_id . '" shift_date="' . date('Y-m-d', strtotime($shift->created_at)) . '" start_time="' . date('h:i', strtotime($shift->start_time)) . '" end_time="' . date('h:i', strtotime($shift->end_time)) . '" area_id="' . $shift->area_id . '">
             <i class="mdi mdi-pencil font-size-18"></i>
             </a>' : '') . (check_action_permission(get_user_role(), 'shift_management', 'shiftSwap') ? '<a class="btn btn-outline-info btn-custom-light btn-sm edit ' . ($shift->is_swap ? 'disabled' : '') . '" title="Swap" href="javascript:void(0)" onclick="swapShift(' . $shift->id . ', \'admin/logistic-management/hunger/rider-shift-swap\')">
             <i class="mdi ' . ($shift->is_swap ? 'mdi-repeat-off' : 'mdi-swap-horizontal-bold') . ' font-size-18"></i>
             </a>' : '') . '<a class="btn btn-outline-info btn-custom-light btn-sm edit" title="View" onclick="admin_view_riderShift(this)" href="javascript:void(0)" shift_name="' . $shift->shift_name . '" rider_name="' . $shift->rider_name . '" shift_date="' . date('d/m/Y', strtotime($shift->created_at)) . '" shift_time="' . date('h:i A', strtotime($shift->start_time)) . ' - ' . date('h:i A', strtotime($shift->end_time)) . '" shift_duration="' . $time_difference . '" area_name="' . $shift->area_name . '">
             <i class="mdi mdi-eye font-size-18"></i>
             </a>' . (check_action_permission(get_user_role(), 'shift_management', 'shiftDelete') ? '<a class="btn btn-outline-danger btn-custom-light btn-sm edit pt-1 pb-1 pr-2 pl-2" title="Delete" onclick="deleteAlert(' . $shift->id . ', \'admin/logistic-management/hunger/rider-shift-delete\')" href="javascript:void(0)">
            <i class="fas fa-trash-alt font-size-18"></i>
            </a>' : '') . '</div>';
			$sub_array[] = $shift->updated_at;

			$data[] = $sub_array;
		}

		$total_records = $this->db->count_all_results('hunger_shift');
	...
	public function shiftAssign()
	{
		...
		try {
			foreach ($shiftData as $shift) {
				$start = new DateTime($shift['start_time']);
				$end = new DateTime($shift['end_time']);
				$time_difference = $start->diff($end)->format('%H:%I:%S');
				...
				if ($shiftId) {
					...
					$this->db->update('hunger_shift', $data, ['id' => $shiftId]);
				} else {
					$data['rider_id'] = $rider_id;
					$this->db->insert('hunger_shift', $data);
				}
			}
	...
	public function getShiftChange()
	{
		...
		$shift_id = $this->input->get('shiftId');
		$edit = $this->db->get_where('hunger_shift', ['id' => $shift_id])->row();
		...
		if ($edit->date < date('Y-m-d')) {
			echo json_encode(['status' => false, 'message' => 'Shift not editable for back date']);
			return;
		}

		$profile = $this->db->select('me.id as emp_id, me.emp_no, me.full_name, me.mobile, me.employee_pic, 
                                      me.designation, me.department, me.nationality, me.employee_arabic_name, 
                                      mjt.name as job_title, md.name as department_name, mei.driving_license_number, 
                                      mv.vehicle_type, mv.vehicle_no, mv.vehicle_model, mv.gps_device_serial, 
                                      mvm.make_name, mn.name as nationality_name')
			->from('master_employee me')
			->join('master_job_title mjt', 'me.designation = mjt.id', 'left')
			->join('master_department md', 'me.department = md.id', 'left')
			...
			->join('master_vehicles mv', 'me.id = mv.alloted_user', 'left')
			->join('mater_van_make mvm', 'mv.vehicle_make = mvm.id', 'left')
			->where('me.id', $edit->rider_id)
			->get()
			->row();

		$data = [
			'edit' => $edit,
			'shifts' => $this->db->get('hunger_shift_master')->result(),
			'areas' => $this->db->where('status', '1')->get('hunger_area_master')->result(),
			'profile' => $profile
		];

		if ($profile) {
			$profile_html = $this->load->view('admin/logistic-management/hunger/components/change_shift', $data, true);
			echo json_encode(['status' => true, 'message' => 'Rider profile fetched', 'profile' => $profile_html]);
		} else {
			echo json_encode(['status' => false, 'message' => 'Profile not found']);
		}
	}
	...
	public function getShiftWeeklyData()
	{
		...
		if ($all_rider) {
			$team_member_ids = $this->db->select('team')->get('hunger_team')->result_array();
			$team_members = array_unique(array_merge(...array_map('json_decode', array_column($team_member_ids, 'team'))));
		} else {
			$this->db->where('team_leader', $team_leader);

			if ($rider_id) {
				$this->db->where("JSON_CONTAINS(team, '\"$rider_id\"')");
			}

			$team = $this->db->get('hunger_team')->row();
			...
			$team_members = json_decode($rider_id ? $rider_id : $team->team);
		}
	...
	public function getShiftDailyData()
	{
		...
		if ($all_rider) {
			...
			$this->db->where('team_leader', $leader_id);
			...
			$riders_id = json_decode($rider_id ? $rider_id : $team->team);
		}
