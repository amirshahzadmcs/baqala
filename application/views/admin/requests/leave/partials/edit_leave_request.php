...
<?php
	// Assuming $leave_requests is the object passed to the view
	$leaveRequest = $leave_requests;

	// Decode the employees_ids and approver JSON strings
	$employeesIds = json_decode($leaveRequest->employees_ids, true);
	$approvers = json_decode($leaveRequest->approver, true);
?>
<?php echo form_open("admin/hr-module/request/update-leave", array("id" => "editLeaveForm", "enctype" => "multipart/form-data", "class" => "form-label-left", "data-parsley-validate" => "")); ?>
<input type="hidden" name="leave_id" value="<?php echo $leaveRequest->id; ?>" />

<div class="row tab-inner-section m-1 py-4">
    <div class="col-md-12 col-sm-12 mb-3 form-group">
        <label for="name">Name</label>
        <input type="text" class="form-control" name="name" id="name" value="<?php echo $leaveRequest->name; ?>" placeholder="Name" required />
    </div>
	<?php $approvalDetail = json_decode($leaveRequest->approval_detail);?>
    <div class="col-md-12 col-sm-12 mb-3 form-group">
        <label for="leave_types">Leave Type</label>
        <select name="leave_types" id="leave_types" class="form-select select2">
            <option value="0" <?php echo ($leaveRequest->leave_types == '0') ? 'selected' : ''; ?>>All Type</option>
            <?php foreach ($leave_lists as $leaveType): ?>
				<option value="<?= $leaveType['id'] ?>" <?= $approvalDetail->leave_types == $leaveType['id'] ? 'selected' : '' ?>>
					<?= $leaveType['name'] ?>
				</option>
			<?php endforeach; ?>
        </select>
    ...
    <div class="col-md-12 col-sm-12 mb-3 form-group">
        <input class="form-check-input" type="checkbox" id="is_applicable_to_all" name="is_applicable_to_all" <?= $leaveRequest->is_applicable_to_all == 'yes' ? 'checked' : '' ?> />
        <label class="form-check-label" for="is_applicable_to_all">Apply this policy to all employees in the company?</label>
    </div>

    <div class="col-md-12 col-sm-12 mb-3 form-group">
        <div id="addEmployeeContainer" style="display: <?php echo ($leave_requests->is_applicable_to_all == 'yes') ? 'none' : 'block'; ?>;">
            <p><small>Selected Employees</small></p>
            <span><i class="mdi mdi-account-multiple-outline font-size-20" style="vertical-align: middle;"></i> 
            <span id="employeeCount" class="text-primary ms-2"><?php echo count($employeesIds); ?></span><span class="text-primary ms-2">Employees</span></span>
    ...
    <div class="col-md-12 col-sm-12 mb-3 form-group">
		<div class="approver-section">
			...
			<ul id="approverList" class="list-group connectedSortable">
				<!-- Loop through the current approvers -->
				<?php foreach($approvers as $index => $approver): ?>
				<li class="list-group-item approver-item">
					<span class="badge bg-light me-2"><?php echo ($index + 1); ?></span>
					<select class="form-select approver-select" name="approver[]">
						<option value="" disabled>Select Employee</option>
						<?php 
							if($approver['approver_type'] == 'employee'){ 
							$empDetail = employeeDetailHelper($approver['approver_list']);
						?>
							<option value="<?php echo $empDetail->id; ?>" <?php echo ($empDetail->id == $approver['approver_list']) ? 'selected' : ''; ?>>
								<?php echo $empDetail->full_name; ?>
							</option>
						<?php } ?>
						<option value="Line Manager" <?php echo ('Line Manager' == $approver['approver_list']) ? 'selected' : ''; ?>>Line Manager</option>
						<option value="Department Head" <?php echo ('Department Head' == $approver['approver_list']) ? 'selected' : ''; ?>>Department Head</option>
					</select>
					<input type="hidden" name="approver_list[]" value="<?php echo $approver['approver_list']; ?>" />
					<input type="hidden" name="approver_type[]" value="<?php echo $approver['approver_type']; ?>" />
					<input type="hidden" class="approver-index" name="approver_index[]" value="<?php echo ($index + 1); ?>" />
...
<script>
	$(document).ready(function() {
		var selectedEmployees = <?php echo json_encode($employeesIds); ?> || [];

		// Similar functionality for handling employee selection, approvers, and form submission.
		// Reuse the previous script but adjust for editing.
		$('#leave_types').on('change', function() {
			var selectedValues = $(this).val();
			if (selectedValues.includes('all_type')) {
				$(this).val(['all_type']).trigger('change.select2');
			} else {
				$(this).find('option[value="all_type"]').prop('selected', false).trigger('change.select2');
			}
		});
		...
		// Checkbox change for applying to all employees
		...
		// 		$(this).find('.approver-index').val(index + 1); // Update hidden index
		// 	});
		// }
		...
		// Initialize employee list and approver select2 fields (as per your original script)
		initializeEmployeeList();
		initializeSelect2WithFooter($('.approver-select'));

		// Add new approver row functionality
		$('#addApprover').on('click', function () {
			...
			var newApprover = `
				<li class="list-group-item approver-item">
					...
					<input type="hidden" name="approver_type[]" value="designation" />
		...
		$(document).on('change', '.approver-select', function () {
			...
			if (isDuplicate) {
				...
				$(this).closest('.approver-item').find('input[name="approver_list[]"]').val(selectedValue);
				$(this).closest('.approver-item').find('input[name="approver_type[]"]').val('designation');
			}
		});
	...
	function initializeSelect2WithFooter(element) {
		element.select2({
			...
			templateSelection: function (data, container) {
				$(data.element).attr('data-badge', data.text);
				return data.text;
			}
		});
	}

	// Function to update approver indexes dynamically
	function updateApproverIndexes() {
		$('#approverList .approver-item').each(function (index) {
