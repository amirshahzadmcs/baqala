        ...
        <div class="row align-items-center">
            ...
            <div class="col-sm-6">
                <div class="float-end d-sm-block">
                    <button type="button" class="btn btn-custom-success btn-sm pull-right" title="Add" id="addTransferBtn"><i class="fa fa-plus"></i> Add Approval Cycle</button>
                </div>
								...
								<tr>
									...
									<td class="p-0">
										<table class="table custom-hover-table" cellpadding="5" cellspacing="0">
											...
											<tbody>
												<?php 
												if($transfer_approvals){
												foreach ($transfer_approvals as $result) {
    ...
    <div class="modal-dialog modal-dialog-aside">
		<div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title mt-0" id="leaveModalLabel">New Transfer Approval Cycles</h5>
...
<div class="modal fade fixed-left employeeSelectionModal" aria-labelledby="#selectEmployeeModalLabel" style="display: none;" aria-hidden="true">
    <div class="modal-dialog modal-dialog-aside">
		<div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title mt-0" id="selectEmployeeModalLabel">Transfer</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
...
$('#addTransferBtn').on('click', function() {
    ...
    $.ajax({
        url: '<?php echo base_url('admin/hr-module/request/transfer/add-form'); ?>',
        method: 'GET',
        success: function(responseHtml) {
			...
			//initializeSortable();
			$('#leaveModalLabel').html('Add Transfer Approval Cycles');
			var footerHtml = `<div class="row">
					<div class="col-md-12">
						<button form="addTransferForm" type="submit" class="btn btn-success btn-md">Save</button>
...
$('.edit-leave-btn').on('click', function() {
    ...
    // Make the AJAX request
    $.ajax({
        url: '<?php echo base_url('admin/hr-module/request/transfer/edit-form'); ?>',
        method: 'GET',
        ...
        success: function(responseHtml) {
			...
			$('.leave-modal-body').html(responseHtml);
			$('.select2').select2({dropdownCssClass: 'custom-dropdown'});
			$('#leaveModalLabel').html('Edit Transfer Approval Cycles');
			var footerHtml = `<div class="row">
					<div class="col-md-12">
						<button form="editTransferForm" type="submit" class="btn btn-success btn-md">Save</button>
						<button type="button" data-bs-dismiss="modal" aria-label="Close" class="btn btn-outline-danger btn-md ms-2">Cancel </button>
					</div>
				</div>`;
			$('.leaveModal .modal-footer').html(footerHtml);
...
$(document).on('click', '.delete-btn2', function() {
	...
	Swal.fire({
		...
		if (result.isConfirmed) {
			$.ajax({
				url: '<?php echo base_url('admin/hr-module/request/transfer/delete'); ?>',
				method: 'POST',
