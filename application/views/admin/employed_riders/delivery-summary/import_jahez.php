		...
		<div class="row align-items-center">
			...
			<div class="col-sm-6">
				<div class="float-end d-sm-block">
					<a class="btn btn-sm btn-custom-white pull-right" title="Back" href="<?php echo base_url('admin/employed-rider/jahez/index'); ?>"><i class="fa fa-reply"></i> Back</a>
							...
							<div class="col-md-6">
								<form id="form-upload-user" method="post" autocomplete="off">
									...
									<div class="form-group">
										<label class="control-label">Choose File <small class="text-danger">*</small> (<a href="<?php echo base_url('admin_assets/samples/Jahez_Sample_File.xlsx'); ?>" target="_blank" download>Download Sample File</a>)</label>
...
<script>
    $(document).ready(function() {
        $("body").on("submit", "#form-upload-user", function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: "<?php echo base_url('admin/employed-rider/jahez/import') ?>",
                data: data,
                dataType: 'json',
                contentType: false,
                cache: false,
                ...
                success: function(result) {
                    ...
                    if(result.type == 'success') {
                        $(".ajax-response").html('<div class="alert alert-success" role="alert">'+ result.message +'</div>');
						$('#form-upload-user #file').val('');
                    } else {
                        $(".ajax-response").html('<div class="alert alert-danger" role="alert">'+ result.message +'</div>');
                    }
                    $(".user-loader").hide();
                },
