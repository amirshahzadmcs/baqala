...
<?php
	// Assuming $loan_requests is the object passed to the view
	$loanRequest = $loan_requests;

	// Decode the employees_ids and approver JSON strings
	$employeesIds = json_decode($loanRequest->employees_ids, true);
	$approvers = json_decode($loanRequest->approver, true);
?>
<?php echo form_open("admin/hr-module/request/update-loan", array("id" => "editLoanForm", "enctype" => "multipart/form-data", "class" => "form-label-left", "data-parsley-validate" => "")); ?>
<input type="hidden" name="loan_id" value="<?php echo $loanRequest->id; ?>" />

<div class="row tab-inner-section m-1 py-4">
    <div class="col-md-12 col-sm-12 mb-3 form-group">
        <label for="name">Name</label>
        <input type="text" class="form-control" name="name" id="name" value="<?php echo $loanRequest->name; ?>" placeholder="Name" required />
    </div>
	<?php $approvalDetail = json_decode($loanRequest->approval_detail);?>

    <div class="col-md-12 col-sm-12 mb-3 form-group">
        <input class="form-check-input" type="checkbox" id="is_applicable_to_all" name="is_applicable_to_all" <?= $loanRequest->is_applicable_to_all == 'yes' ? 'checked' : '' ?> />
        <label class="form-check-label" for="is_applicable_to_all">Apply this policy to all employees in the company?</label>
    </div>

    <div class="col-md-12 col-sm-12 mb-3 form-group">
        <div id="addEmployeeContainer" style="display: <?php echo ($loanRequest->is_applicable_to_all == 'yes') ? 'none' : 'block'; ?>;">
...
<script>
	$(document).ready(function() {
		...
		$('#employeeSelectionButton').on('click', function() {
			$.ajax({
				url: '<?php echo base_url('admin/hr-module/request/get-employee-list'); ?>',
				method: 'GET',
		...
		$(document).on('change', '.approver-select', function () {
			...
			var currentApproverItem = $(this).closest('.approver-item');
