...
<?php
	...
	$warningRequest = $warning_approval;

	// Decode the employees_ids and approver JSON strings
	$employeesIds = json_decode($warningRequest->employees_ids, true);
	$approvers = json_decode($warningRequest->approver, true);
?>
<?php echo form_open("admin/hr-module/request/notice-warning/update", array("id" => "editNoticesForm", "enctype" => "multipart/form-data", "class" => "form-label-left", "data-parsley-validate" => "")); ?>
<input type="hidden" name="request_id" value="<?php echo $warningRequest->id; ?>" />

<div class="row tab-inner-section m-1 py-4">
    <div class="col-md-12 col-sm-12 mb-3 form-group">
        <label for="name">Name</label>
        <input type="text" class="form-control" name="name" id="name" value="<?php echo $warningRequest->name; ?>" placeholder="Name" required />
    </div>
	<?php $approvalDetail = json_decode($warningRequest->approval_detail);?>

    <div class="col-md-12 col-sm-12 mb-3 form-group">
        <input class="form-check-input" type="checkbox" id="is_applicable_to_all" name="is_applicable_to_all" <?= $warningRequest->is_applicable_to_all == 'yes' ? 'checked' : '' ?> />
        <label class="form-check-label" for="is_applicable_to_all">Apply this policy to all employees in the company?</label>
    </div>

    <div class="col-md-12 col-sm-12 mb-3 form-group">
        <div id="addEmployeeContainer" style="display: <?php echo ($warningRequest->is_applicable_to_all == 'yes') ? 'none' : 'block'; ?>;">
...
<script>
	$(document).ready(function() {
		...
		$('#editNoticesForm').on('submit', function (e) {
			e.preventDefault();
