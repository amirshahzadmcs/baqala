...
class Keeta extends CI_Controller
{
	...
	public function __construct()
	{
		...
		// Load Model
		$this->load->model('admin/logistic-management/aggregators/keeta/Keeta_model', 'keeta_model');
	...
	{
		if ($this->action && !check_action_permission(get_user_role(), 'keeta_monthly_performance', $this->action)) {
		...
		$data['keeta_summary'] = $this->keeta_model->getSummary();
		$this->load->view("admin/logistic-management/aggregators/keeta/index", $data);
	}
	...
	public function import_file()
	{
		...
		try {
			...
			$existing_records = $this->keeta_model->checkDuplicateInBulk($summary_date);
			if (!empty($existing_records)) {
				...
				$this->db->insert('maha_keeta_monthly', $data);
				...
				foreach ($sheet_data as $val) {
					...
					if (!empty($val[3])) { // Replace with necessary column checks
						$batch_data[] = [
							...
							"note" => $val[7],
							"detail_amount" => $val[8],
							"total_payable_amount" => $val[9],
							"ticket_id" => $val[10],
							"violation_id" => $val[11],
							"violation_type" => $val[12],
							"punishment_methods" => $val[13],
							"created_at" => $this->datetime,
							"updated_at" => $this->datetime,
						];
	...
	{
		$result = $this->db->insert_batch('maha_keeta_monthly_summary', $batch_data);
	...
	public function detail($id)
	{
		...
		if ($id) {
			...
			$config['base_url'] = base_url("admin/logistic-management/invoices/keeta/detail/$id");
			$config['suffix'] = "&search=" . urlencode($search) . "&per_page=" . $perPage;
			$config['first_url'] = $config['base_url'] . '?page=1' . $config['suffix'];
			$config['page_query_string'] = TRUE;
			$config['query_string_segment'] = 'page';
			$config['total_rows'] = $this->keeta_model->summaryDetailCount($id, $search);
			...
			$summaryData = $this->keeta_model->get_detail($id);
			$summaryDetails = $this->keeta_model->summaryDetail($id, $search, $perPage, $page);
			//dd($summaryDetails);

			if ($isAjax) {
				$html = $this->load->view('admin/logistic-management/aggregators/keeta/components/summary-data', ['summaryDetails' => $summaryDetails['data'], 'start' => $data['start'], 'availableTableColumns' => $summaryDetails['visible_columns'], 'visibleTableColumns' => $summaryDetails['visible_columns']], TRUE);
				echo json_encode([
					'html' => $html,
			...
			//dd($data);
			$this->load->view('admin/logistic-management/aggregators/keeta/detail', $data);
		} else {
			$this->session->set_userdata('msg_info', "2--Invalid request!");
			redirect('admin/logistic-management/invoices/keeta/list');
		}
	}
	...
	public function print_monthly_performance()
	{
		...
		$data['report'] = $this->keeta_model->get_detail($id);
		$data['report_data'] = $this->keeta_model->printPerformanceReport($id, $search, $column_type, $file_format);
		...
		$pdf->SetTitle('Keeta Monthly Performance Report -' . $data['month_of']);
		$pdf->SetSubject('Keeta Monthly Performance Report -' . $data['month_of']);
		$pdf->SetKeywords('Maha Al Fala, PDF, Keeta Monthly Performance Report');

		// print_r($data);exit();
		// remove default header/footer
		$pdf->setPrintHeader(true);
		$pdf->SetPrintFooter(true);
		$htmlHeader = $this->load->view('admin/logistic-management/aggregators/keeta/print/keeta_monthly_header', $data, true);
		$htmlHeader2 = $this->load->view('admin/logistic-management/aggregators/keeta/print/keeta_monthly_header', $data, true);
		$pdf->setHtmlHeader($htmlHeader);
		$pdf->setHtmlHeader2($htmlHeader2);

		$lastFooter = $this->load->view('admin/logistic-management/aggregators/keeta/print/footer', $data, true);
		...
		// Arabic and English content
		$htmlcontent = $this->load->view('admin/logistic-management/aggregators/keeta/print/keeta_monthly_report', $data, true);
		$pdf->WriteHTML($htmlcontent, true, 0, true, 0);
		//Close and output PDF document
		$pdf->Output('BS-Keeta-Station-Report-' . $data['month_of'] . '.pdf', 'I');
	...
	public function export_monthly_performance()
	{
		...
		$filename = "Keeta-Performance-Report-" . $data['month_of'] . ".xlsx";
		...
		foreach ($report_data as $detail) {
			...
			$totalPayableAmount += (int) ($detail['total_payable_amount'] ?? 0);
		}
		...
		$sheet->setCellValue('A1', 'Keeta Monthly Delivery Report - ' . $data['month_of']);

		// Set Summary Header
		$sheet->setCellValue('A2', "No's Riders")->setCellValue('B2', $nosRiders);
		$sheet->setCellValue('C2', "Total Payble Amount")->setCellValue('D2', $totalPayableAmount);
		...
		foreach ($data['report_data']['available_columns'] as $column) {
			$cleanedHeader = strtoupper(str_replace(['mkms.', 'me.'], '', $column));
