            ...
            <div class="col-sm-6">
                <div class="page-title">
                    ...
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">Change Profession</li>
                    </ol>
                </div>
            </div>
            <div class="col-sm-6">
                <div class="float-end d-sm-block">
                    <button type="button" class="btn btn-custom-success btn-sm pull-right" title="Add" id="addProfessionBtn"><i class="fa fa-plus"></i> Add Approval Cycle</button>
                </div>
								...
								<tr>
									...
									<td class="p-0">
										<table class="table custom-hover-table" cellpadding="5" cellspacing="0">
											...
											<tbody>
												<?php 
												if($profession_approvals){
												foreach ($profession_approvals as $result) {
...
<div class="modal fade fixed-left professionModal" aria-labelledby="#professionModalLabel" style="display: none;" aria-hidden="true">
    <div class="modal-dialog modal-dialog-aside">
		<div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title mt-0" id="professionModalLabel">New Profession Approval Cycles</h5>
...
<div class="modal fade fixed-left employeeSelectionModal" aria-labelledby="#selectEmployeeModalLabel" style="display: none;" aria-hidden="true">
    <div class="modal-dialog modal-dialog-aside">
		<div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title mt-0" id="selectEmployeeModalLabel">Change Profession</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
...
$('#addProfessionBtn').on('click', function() {
    ...
    $.ajax({
        url: '<?php echo base_url('admin/hr-module/request/change-profession/add-form'); ?>',
        method: 'GET',
        success: function(responseHtml) {
            $('.professionModal').modal('show');
            $('.profession-modal-body').html(responseHtml);
			$('.select2').select2({dropdownCssClass: 'custom-dropdown'});
			//initializeSortable();
			$('#professionModalLabel').html('Add Profession Approval Cycles');
			var footerHtml = `<div class="row">
					<div class="col-md-12">
						<button form="addProfessionForm" type="submit" class="btn btn-success btn-md">Save</button>
						<button type="button" data-bs-dismiss="modal" aria-label="Close" class="btn btn-outline-danger btn-md ms-2">Cancel </button>
					</div>
				</div>`;
			$('.professionModal .modal-footer').html(footerHtml);
        },
...
$('.edit-leave-btn').on('click', function() {
    ...
    // Make the AJAX request
    $.ajax({
        url: '<?php echo base_url('admin/hr-module/request/change-profession/edit-form'); ?>',
        method: 'GET',
        ...
        success: function(responseHtml) {
			...
			$('.profession-modal-body').html(responseHtml);
			$('.select2').select2({dropdownCssClass: 'custom-dropdown'});
			$('#professionModalLabel').html('Edit Profession Approval Cycles');
			var footerHtml = `<div class="row">
					<div class="col-md-12">
						<button form="editProfessionForm" type="submit" class="btn btn-success btn-md">Save</button>
						<button type="button" data-bs-dismiss="modal" aria-label="Close" class="btn btn-outline-danger btn-md ms-2">Cancel </button>
					</div>
				</div>`;
			$('.professionModal .modal-footer').html(footerHtml);
...
$(document).on('click', '.delete-btn2', function() {
	...
	Swal.fire({
		...
		if (result.isConfirmed) {
			$.ajax({
				url: '<?php echo base_url('admin/hr-module/request/change-profession/delete'); ?>',
				method: 'POST',
				data: { transfer_id: requestId },
				dataType: 'json',
