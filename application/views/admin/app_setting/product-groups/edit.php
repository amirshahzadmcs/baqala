	...
	<div class="page-content-wrapper">
		 <div class="row">
			<div class="col-12">
				<div class="card">
					<div class="card-body">
						<?php echo form_open("admin/app/product-groups/update", array("id"=>"bannerForm", "enctype"=>"multipart/form-data", "class"=>"form-label-left", "data-parsley-validate"=> "")); ?>
						...
						<fieldset>
							...
							<div class="row">
								...
								<div class="col-md-6 mb-3 form-group">
									...
									<input type="text" class="form-control" id="group_title" name="group_title" maxlength="255" value="<?php echo $group_title; ?>" required />
								</div>

								<div class="col-md-6 mb-3 form-group">
									<label for="group_title_arabic" class="float-end">Group Name Title (Arabic)</label>
									<input type="text" class="form-control rtl-input" id="group_title_arabic" name="group_title_arabic" value="<?php echo $group_title_arabic; ?>" maxlength="255" />
								</div>

								<div class="col-md-6 mb-3 form-group">
									<label for="group_url">Group Url<span class="text-danger">*</span></label>
									<input type="text" class="form-control" id="group_url" name="group_url" maxlength="99" value="<?php echo $group_url; ?>" onBlur="checkDuplicateUrl()" required />
									<p class="mt-1 mb-0 res-msg"></p>
								</div>
								<div class="col-md-6 mb-3 form-group">
									<label for="sort_order">Sort Order <span class="text-danger">*</span></label>
									<input type="text" class="form-control" id="sort_order" name="sort_order" onkeypress="return numerics(event);" value="<?php echo $sort_order; ?>" required maxlength="5" />
								</div>
									...
									<table class="table table-striped jambo_table table-bordered mb-0" id="item_table" width="100%" border="1" cellspacing="0" cellpadding="5">
										...
										<tbody>
											...
											foreach($products as $product){ ?>
												<?php $p_image = ($product['image'] == "") ? "images/image-not-available.jpg" : $product['image'];?>
												<tr id="item-row<?php echo $item_row; ?>">
													<td class="text-right"><button type="button" onclick="remove_item(<?php echo $item_row;?>)" data-toggle="tooltip" title="Remove" class="btn btn-danger btn-sm"><i class="fa fa-minus-circle"></i></button></td>
													<td class="text-left"><img src="<?php echo base_url($p_image);?>" class="img-fluid rounded shadow-sm mr-3" width="40px" /></td>
													<td class="text-left"><input type="hidden" name="products[size_id][]" value="<?php echo $product['id'];?>" required /><input type="hidden" name="products[prod_id][]" value="<?php echo $product['prod_id'];?>" required /><input type="text" name="products[item_sku][]" class="form-control" value="<?php echo $product['parent_sku'] .'-'. $product['sku'];?>" readonly="readonly" /></td>
													<td class="text-left"><input type="text" name="products[barcode][]" class="form-control" value="<?php echo $product['barcode'];?>" readonly="readonly" /></td>
													<td class="text-left"><input type="text" name="products[item_description][]" id="item-list" value="<?php echo $product['name'];?>" class="form-control" readonly="readonly" /></td>
													<td class="text-left"><input type="number" min="1" max="1000" name="products[sort_order][]" value="<?php echo $product['sort_order'];?>" class="form-control" /></td>
												</tr>
